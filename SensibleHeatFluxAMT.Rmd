---
title: Measuring the Flux of Sensible Heat With Airborne Thermometers
...
abstract: |
  ...
  especially measuring the turbulent transport of thermal energy, conventionally
  referred to as the flux of sensible heat. This paper uses a new method
  to determine the frequency-domain transfer function that characterizes the
  time response of standard airborne sensors, and it uses the result to
  improve measurements of the flux density of thermal energy. Thermometers used
  on research aircraft sense the "recovery temperature" that is
  produced when air is compressed on approach to the sensor. The magnitude
  of the temperature change varies with airspeed but also with changes
  in the wind. The transfer function is found by observing the response
  to turbulent fluctuations in this temperature change, termed here
  "dynamic heating." Flight segments were used where such turbulent
  fluctuations were the dominant cause of fluctuations in the measurement.
  ...
  parameters in those differential equations with low uncertainty. Examples
  ...
  too low, but the proposed correction procedure removes this error
  and results in reliable measurements with negligible error attributable
  to the response time of the sensor. An additional result of the study
  is the identification of a source of error, prevalent in most archived
  data files produced by research aircraft, that results from failure
  to take into account the response of the sensor when correcting for
  dynamic heating. 
...
availability:
  ...
  codedata: |
    ...
    Laboratory, at this web site: \href{https://data.eol.ucar.edu}{data.eol.ucar.edu}
    ...
    program, "SensibleHeatFluxAMT.Rnw" is archived on "GitHub"
    in the directory at 
    ...
    the "chunks" subdirectory, so the full directory should be downloaded
    ...
    and the generation of this document. A package named Ranadu [@Ranadu2.6],
...
acknowledgements: |
  ...
  \citet{WECAN2018}) were collected in research projects (\citet{wood2011vamos},
  ...
  information can be found at [this URL](https://www.eol.ucar.edu/field_projects/).
  ...
  were mostly performed using R (\citet{Rlanguage}), with RStudio (\citet{RStudio2012})
  ...
  function from the "rmutil" package (\citet{runge.kutta}). A special
  R package (\citet{Ranadu2.6}) was used extensively in the analysis.
...
to resolve the spectrum of contributions to the flux. Various reviews
of priorities for research in atmospheric science have called attention
to the important roles that fluxes of various quantities play in climate
science and have advocated increased focus on those fluxes; e.g.,
@NAP6021. (The flux is perhaps better termed the flux of thermal
energy or enthalpy or dry static energy and is properly described as a 
flux density because it characterizes
the rate of transport of energy per area, but "sensible-heat flux"
has become the standard term and will be used here.)

The basis for the measurement of the flux of sensible heat ($F_{s})$
by eddy correlation is given by, e.g., @hartmann201695:
\begin{equation}
...
errors in this measurement, it therefore is essential to characterize
...
@PayneEtAl1994 provided a particularly illuminating analysis
...
that of @mccarthy1973method but poor agreement with the empirical
...
is in contact with the wire. Friehe and Khelif [@FrieheKhelif1992],
following other prior work including that of @rodi1972analysis
and @mccarthy1973method, suggested representing the two-time-constant
...
Following the approach of @PayneEtAl1994, the time response
of the sensor will be represented here by two coupled differential
equations, one that describes the response of the support on which
...
are equivalent to the expressions for the same coefficients given by \eqref{eq:C1} and
\eqref{eq:C2}. This demonstrates that the equations
...
and the cosine of the phase so at \unit{10\,Hz} the error is about 86\%,
but even at \unit{1\,Hz} the error is about 28\%. 

\begin{figure}
```{r LTsolution, include = TRUE, out.width = "8.3cm"}
...
heating of temperature sensors is discussed for example by @BangeEtAl2013.ch2
...
parameters like $a$, $\tau_{1}$ and $\tau_{2}$ that fit the predictions
...
given by \eqref{eq:responseAmp} and \eqref{eq:responsePhase}.
...
are included in an NCAR Technical Note [@Cooper2016ncartn],
...
of temperature from the GV is about \unit{0.3^{\circ}C} and referenced
@CooperEtAl2014 for supporting evidence. This limit applies
...
field projects are described by @wood2011vamos, @albrecht2019CSET
and @mcfarquhar2014southern, respectively. All included low-level
...
the estimate became stable after only one iteration.
...
[@wood2011vamos], which studied low-level clouds over the
...
using the R routine "spec.pgram()" with 25-point modified Daniell
smoothing, and the results for all six segments were averaged in `r round(SB,0)`
...
\begin{figure}
```{r Vphase, include=TRUE, echo=FALSE, out.width = "8.3cm"}
...
relative to dynamic heating.]{(top): Phase of measured recovery temperature relative to dynamic
...
those limits. Below \unit{0.1\,Hz} it appears likely that the sensor
...
## results. Decided that they don't merit a separate result for the GV.
DSX <- getNetCDF(
...
DS11 <- getNetCDF(
    file.path(DataDirectory(), 'SOCRATES/SOCRATESrf11h.nc'),
    standardVariables(c('ATF1', 'ATH1')),
    34850,
    35900
  )
...
if (CorrectT) {
  ...
  fS <- function(y, i) {
    # Eq. Ts3
    ((1 / a) * (tau1 * DS11$DTMDT[i] + DS11$RTF1[i] - (1 - a) * y) - y) / 
      (Rate * tau2)
  }
  DS11$Ts <- rk4.integrate (fS, DS11$Ts[1], 1:nrow(DS11))
  DS11$RT <-
    (1 / a) * (tau1 * DS11$DTMDT + DS11$RTF1 - (1 - a) * DS11$Ts)
} else {
  DS11$RT <- DS11$RTF1
}
...
SB <- 90
Z <- capture.output (
       Vr <- SmoothInterp(detrend (DS11[, c('Time', 'RT')]), .Length = 0))
Z <- capture.output (
       VrC <- SmoothInterp(detrend (DS11[, c('Time', 'Q')]), .Length = 0))
Rate <- attr(DS11, 'Rate')
vcv <- cbind(ts(Vr, frequency = Rate), ts(VrC, frequency = Rate))
P <- spec.pgram(
  vcv,
...
for (RX in list(RX1)) {
  for (i in 2:nrow(RX)) {
    ...
    nphaseSF[RX$BIN.1[i]] <- nphaseSF[RX$BIN.1[i]] + 1
    phase2SF[RX$BIN.1[i]] <-
      phase2SF[RX$BIN.1[i]] + RX$P.phase[i] ^ 2
    arSF[RX$BIN.1[i]] <-
      arSF[RX$BIN.1[i]] + sqrt(RX$P.spec...1.[i] / RX$P.spec...2.[i])
    ar2SF[RX$BIN.1[i]] <-
      ar2SF[RX$BIN.1[i]] + (RX$P.spec...1.[i] / RX$P.spec...2.[i])
...
minFn <- function(V) {
  ...
  for (i in 1:nrow(dfSF)) {
    Chisq <- Chisq + 
             (dfSF$mphase[i] - LTphase(dfSF$frq[i], Param2)$Phase)^2 /
             dfSF$sdp[i]^2
  }
  for (i in 1:nrow(dfrSF)) {
    Chisq <- Chisq + 
             (dfrSF$mrspec[i] - LTphase(dfrSF$frq[i], Param2)$Amp)^2 /
             dfrSF$sdr[i]^2
  }
  return (Chisq)
}
ASF <-
  optim (
    c(Param2$a, Param2$tau1, Param2$tau2),
    minFn,
    method = 'L-BFGS-B',
    lower = 0,
    upper = 1,
...
g <-
  g + geom_errorbar(aes(ymin = mphase - sdp, ymax = mphase + sdp), 
                    width = 0.06)
g <-
  g + geom_path(data = dfSF, aes(x = frq, y = Phase), col = 'forestgreen')
g <-
  g + geom_path(data = dfAPSF, aes(x = Time, y = Phase), col = 'darkorange')
g + theme_WAC(1)
...
g <- g + annotation_logticks(sides = "bt")
g <- g +
     geom_errorbar(aes(ymin = mrspec - sdr, ymax = mrspec + sdr), 
                    width = 0.06)
g <- g + 
     geom_path(data = dfAPSF, aes(x = Time, y = Amp), col = 'darkorange')
g <- g + 
     geom_path(data = dfSF, aes(x = frq, y = Amp), col = 'forestgreen')
suppressWarnings(print(g + ylim(0, 1.3) + theme_WAC(1)))
print(bestSF)
rhozero <- 1013.25 * 100 / (287.05 * 288.15)
MRHOGV <-
  MachNumber(DS11$PSXC, DS11$QCXC) * 
    DS11$PSXC * 100 / (287.05 * (273.15 + DS11$ATX)) / rhozero
...
a little smaller than found for the C-130 ($\tau_{1}$=`r round(ParGV$tau1, 3)`\unit{\,s}
and $\tau_{2}$=`r round(ParGV$tau2, 2)`\unit{\,s}). This might be expected
...
```{r SOCrf08, include=FALSE}

DS <- getNetCDF('/Data/SOCRATES/SOCRATESrf08h.nc',
                standardVariables(c(
                  'ATF1', 'ATH1', 'ATH2', 'RTF1', 'RTH1', 'RTH2'
                )),
                45600,
                50100)
...
SB <- 50
Z <- capture.output (
       Vr <- SmoothInterp(detrend (DS[, c('Time', 'RTH1')]), .Length = 0))
Z <- capture.output (
       VrB <- SmoothInterp(detrend (DS[, c('Time', 'RTH2')]), .Length = 0))
Z <- capture.output (
       VrF <- SmoothInterp(detrend (DS[, c('Time', 'RTF1')]), .Length = 0))
Z <- capture.output (
       VrC <- SmoothInterp(detrend (DS[, c('Time', 'Q')]), .Length = 0))
Rate <- attr(DS, 'Rate')
vcv <- cbind(ts(Vr, frequency = Rate), ts(VrC, frequency = Rate))
P <- spec.pgram(
  vcv,
...
for (i in 2:nrow(RXHS)) {
  ...
  phase2HS[RXHS$BIN.1[i]] <-
    phase2HS[RXHS$BIN.1[i]] + RXHS$P.phase[i] ^ 2
  ...
  phaseHSB[RXHSB$BIN.1[i]] <-
    phaseHSB[RXHSB$BIN.1[i]] + RXHSB$P.phase[i]
  nphaseHSB[RXHSB$BIN.1[i]] <- nphaseHSB[RXHSB$BIN.1[i]] + 1
  phase2HSB[RXHSB$BIN.1[i]] <-
    phase2HSB[RXHSB$BIN.1[i]] + RXHSB$P.phase[i] ^ 2
  ...
  phaseHSF[RXHSF$BIN.1[i]] <-
    phaseHSF[RXHSF$BIN.1[i]] + RXHSF$P.phase[i]
  nphaseHSF[RXHSF$BIN.1[i]] <- nphaseHSF[RXHSF$BIN.1[i]] + 1
  phase2HSF[RXHSF$BIN.1[i]] <-
    phase2HSF[RXHSF$BIN.1[i]] + RXHSF$P.phase[i] ^ 2
...
\begin{tabular}{|c|c|c|}
\hline 
**Project / Flight** & **start {[**UTC{]}} & **end {[**UTC{]}}\tabularnewline
\hline 
\hline 
...
(referenced earlier in connection with the unheated probe) were compiled
into one data set from the flight periods shown in Table~2. An unheated
Rosemount 102E4AL sensor was also onboard, so corrected measurements
from that sensor were used as the reference against which to determine
the gain and phase of the transfer function.

\begin{figure}
```{r GVHARCO, include = TRUE, out.width = "8.3cm"}
...
\caption[The phase and gain for the transfer function characterizing a heated
HARCO temperature sensor. ]{The phase (top) and gain (bottom) for the transfer function characterizing
...
for \{$a,\,\tau_{1},\,\tau_{2}$\} were \{0, `r round(bestSH[2], 2)`\unit{\,s},
...
of about $-38^{\circ}$ at \unit{0.1\,Hz} where the amplitude ratio is still
high (about 0.75). The amplitude decreases to $e^{-1}$ at about \unit{0.34\,Hz},
...
the three-parameter fit, although the fit needs to be modified above about \unit{2\,Hz}
...
\hline 
**sensor** & **$a$** & \textbf{$\tau_{1}$} {[}s{]} & \textbf{$\tau_{2}$} {[}s{]}\tabularnewline
\hline 
\hline 
unheated Rosemount 102E4ALon C-130 & `r round(Param1$a, 2)` & `r round(Param1$tau1, 3)` & `r round(Param1$tau2,2)`\tabularnewline
\hline 
unheated Rosemount 102E4AL on GV & `r format(ParGV$a, digits=2, nsmall=2)` & `r round(ParGV$tau1, 3)` & `r format(ParGV$tau2, digits=2, nsmall=2)`\tabularnewline
\hline 
heated HARCO & `r format(bestSH[1], digits=1, nsmall=1)` & `r format(bestSH[2], digits=2, nsmall=2)` & `r format(bestSH[3], digits=2, nsmall=2)`\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
...
\section{Correcting for Dynamic Heating{\label{sec:Correcting-for-Dynamic}}}
...
measurement of the ambient air temperature. As
...
Data processing should therefore apply a correction that represents
...
respond. The approach followed here is to use the transfer
function so that only the sensor response to dynamic heating is subtracted from the
measurement. This is made possible by the assumed linearity in response
...
dynamic heating and true fluctuations in temperature.

```{r LT, include = FALSE, fig.height=3.7, fig.cap = 'The amplitude and phase for the frequency domain transfer function of a Rosemount 102E4AL temperature sensor.'}
...
sensor response. The filter used here was determined at a frequency
resolution of \unit{(1/600)\,Hz}, then only the initial 101 and trailing
100 coefficients were retained and all other were set to zero. Shifting
the negative-frequency components to the start of the array gave an
eight-second sequence that represented the response of the sensor.
These coefficients were used as a moving-average filter, after which
the result was shifted forward in time by four seconds to compensate
for the delay. The procedure used here is somewhat arbitrary and a
better filter could probably be designed, but this filter functions
reasonably for \unit{25\,Hz} measurements. 
...
\begin{figure}
```{r plotImpulse, include=TRUE, out.width = "8.3cm"}

layout(matrix(1:2, ncol = 1),
       widths = c(8, 8),
       heights = c(6.5, 8))
op <- par (mar = c(2, 4, 1, 1) + 0.1, oma = c(1.1, 0, 0, 0))
dP1 <- data.frame(Time = (1:100) * 0.04,
                  R = Re(GT[1:100]),
                  H = 10 * Re(GTH[1:100]))
plotWAC(
  dP1,
...
dP2 <- data.frame(Time = (0:200) * 0.04,
                  R = Re(GTT),
                  H = 10 * Re(GTTH[51:251]))
plotWAC(
  dP2,
...
the response parameters from Table\ \ref{tab:Parameters}. The impulse
response for the HARCO sensor is multiplied by 10.\protect \\
...
sensor, \unit{0.03\,s}, is smaller than the time between \unit{25\,Hz} samples. The
...
\begin{figure}
```{r Integration, include=TRUE, echo=FALSE, fig.width=4.1, out.width='0.48\\linewidth', fig.show='hold', messages = FALSE}
...
\caption[For an unheated Rosemount 102E4AL sensor, variance spectra for the
...
term. ]{Variance spectra for the unheated Rosemount 102E4AL sensor, for the
same flight segment used for Fig.\ \ref{fig:S15}.\protect \\
Left, (a): The dynamic-heating term ("Q") and the filtered term
...
\unit{25\,Hz} smoothing, integrated, and then resampled to obtain \unit{25\,Hz} measurements.
This indicated that the discrepancy in results is attributable to
accumulating numerical errors in the integration, but the integration
became awkwardly slow when performed at \unit{125\,Hz} resolution. The equivalence
...
\begin{figure}
```{r HarcoQ, include=TRUE, echo=FALSE, fig.height=3.5, fig.width=4.1, out.width='0.48\\linewidth', fig.show='hold', messages = FALSE}
...
tau1 <- ParamSH$tau1 * (MRHO / 0.3) ^ 0.6  
...
\caption[Variance spectra for the heated HARCO sensor: The unmodified dynamic-heating
term and two filtered terms. Also, the air temperature as modified
by filtering the dynamic-heating term.]{Variance spectra for the heated HARCO sensor.\protect \\
...
\begin{figure}
```{r ATATF, include=TRUE, echo=FALSE, fig.height=3,  fig.width=4.1, out.width='0.48\\linewidth', fig.show='hold', messages = FALSE}
...
if (file.exists ('./DVOCALS.Rdata')) {
  ...
  print (ga + annotate(
    'text',
    x = DVOCALS$Time[getIndex(DVOCALS, 80100)] + 0.4,
    y = 15.092,
    label = '(a)',
    size = 5
  ))
...
102E4AL sensor and a heated HARCO sensor. ]{Comparison of the original measured temperature and the same temperature
...
heating to which the sensor does not respond. 
...
Most past studies of temperature corrections (e.g., @mccarthy1973method,
...
temperature. Then the appropriate amplification of signals restores
the fluctuations produced by dynamic heating and those fluctuations
are appropriately removed by subtracting the measured dynamic heating
term from the corrected recovery temperature. 

The procedure used here is as follows:

1. Find the transfer function, as in Sect.\ \ref{sec:theTransferFn}.
...
2. Find the air temperature or, equivalently, the Fourier
...
an appropriate scale factor as specified in \eqref{eq:heatFlux}.

3. Calculate the cumulative cospectrum for the cross-spectrum
of temperature and updraft, with a choice made regarding which frequency
interval to include. This will normally exclude wavelengths greater
than a few kilometers.

4. Calculate the flux from the sum of the resulting cospectrum
...
ParamFS1 <- ParamSF
ParamFS1$tau1 <-
  ParamFS1$tau1 * (mean(MRHO, na.rm = TRUE) / 0.3) ^ -0.6
ParamFS1$tau2 <-
  ParamFS1$tau2 * (mean(MRHO, na.rm = TRUE) / 0.3) ^ -0.6
DS$TASX <- SmoothInterp(DS$TASX, .Length = 0)
DS$Q <- DS$TASX ^ 2 / 2010
DS$ATX <- DS$ATF1
...
boundary layer over the Pacific Ocean. An unheated Rosemount 102E4AL
...
using the appropriate transfer function as determined in Sect.\ \ref{sec:theTransferFn}. 

\begin{figure}
```{r SOCp1, include=TRUE, fig.width=6, fig.height=3.0}
...
the estimated response of the temperature sensor. The dashed black
line shows the frequency limit that corresponds to a wavelength of
\unit{2\,km}.{\label{fig:SOCp1}}}
\end{figure}

The results are shown in Fig.\ \ref{fig:SOCp1}, where the measured
...
1. The cospectrum (blue line) has been smoothed using Daniell smoothing,
...
3. The total flux indicated in the title is that arising from the part
of the flux with frequency above \unit{0.01\,Hz]. This corresponds to a wavelength
of more than \unit{10\,km}. In addition, there is another estimate of
the contribution to the flux from wavelengths below a selected limit,
here \unit{2\,km}. That or a still smaller wavelength limit is often a reasonable
...
4. One additional line is plotted brown and labeled "exceedance."
...
*higher* than the indicated value. At high frequency on a logarithmic
...
The units for exceedance are \unit{W\,m^{-2}}, not \unit{W\,m^{-2}}
...
as it is without correction. @LawsonRodi1992 estimated that,
...
to reconstruct the simulated measurement series. The variance spectrum
was generated to have the following form as expected for an inertial
sub-range:
\begin{equation}
...
an eddy dissipation rate of \unit{10^{-3}\,m^{2}\,s^{-3}},
...
To introduce sensible-heat flux a correlation was introduced between
the simulated temperature and updraft, as follows:

1. In addition to the three-component time series for the wind, another
independent time series with the same slope was generated for the
*air temperature *$T_{a}$ with fluctuations $T_{a}^{\prime}$
scaled smaller by a factor of $S$=\unit{0.2^{\circ}C\,m^{-1}\,s}$
in comparison to the magnitude of the wind fluctuations. 

2. Correlation between the two time series was then introduced by defining
...
3. Finally, the measured recovery temperature was simulated by adding
...
About 15\% of the flux from wavelengths below \unit{2.5\,km} is contributed by frequencies
...
```
\begin{figure}
```{r simF, include=TRUE, fig.height = 4, out.width = "12cm"}
...
digital filter developed in Sect.\ \ref{subsec:A-filter-for}to
...
a factor of ten too low at \unit{10\,Hz}. The measured exceedamce distribution
...
of sensible heat was \unit{39.5\,W\,m^{-2}} and the measured values before
and after correction were 33.2 and \unit{38.5\,W\,m^{-2}}. The correction
...
flux therefore provides some support for the filter developed in Sect.\ \ref{subsec:A-filter-for}
...
including those above 1~Hz. The measured values of sensible-heat
...
at frequencies up to about 10~Hz, so the decrease in the cospectrum
...
work, notably that of \citet{InverarityJTech2000}.

The differential equations \eqref{eq:Ts} and \eqref{eq:Tm} involve
...
of the support, $T_{s}(t)$. Then \eqref{eq:Tm} can be solved to
...
with the best-fit value $a=0$, \eqref{eq:Tm} can't be solved for
...
Because finite-difference estimates introduce high-frequency noise,
...
* Calculate the Fourier transform of the measured time series: $\hat{T}_{m}(\omega)=\mathcal{F}\left(T_{m}(t)\right)$
...
* Use the inverse Fourier transform to find a retrieved estimate of
...
Examples of the corrections produced by these procedures are shown
...
the corrected values for the HARCO sensor are improved significantly
by this correction procedure and are even a reasonable match to the
corrected measurement from the unheated Rosemount.

```{r sampleFFT, include=TRUE, fig.height=4.1, fig.width=4.1, out.width='0.48\\linewidth', fig.show='hold', messages = FALSE, fig.cap = 'Examples of the corrected recovery temperatures compared to the original uncorrected measurements. In each case, the correction based on the differential equations is shown as "corrected\\_DE" and that based on Fourier transforms is labeled "corrected\\_FFT" The dashed green line in the left plot shows the calculated temperature of the support that contacts the sensing wire. The solid green line in the right plot shows the corrected measurement from the unheated sensor for comparison.  (left): An unheated Rosemount 102E4AL sensor. Measurements from VOCALS flight 3.  (right): A heated HARCO sensor. In this case the "corrected\\_DE" result is based on \\eqref{eq:HARCOsoln} while "corrected\\_FFT" uses the fit provided by \\eqref{eq:lfitH}. The time is seconds after 2018-02-24 5:59:00 UTC, SOCRATES flight 15. Mean values have been subtracted to facilitate comparison.'}
...
a <- bestSH[1]
tau1 <-
  bestSH[2] * (MRHO / 0.3) ^ 0.6  ## Note that these are variable
...
CutoffPeriod <- 12.5  ## 2 Hz for 25-Hz samples
