...
abstract: |
  ...
  especially measuring the turbulent transport of dry static energy,
  conventionally
  ...
  improve measurements of the flux density of sensible heat. Thermometers used
...
copyrightstatement: |
  The author's copyright for this publication is transferred to the University Corporation for Atmospheric Research, Boulder CO, USA.
...
@NAP6021. (The sensible-heat flux is perhaps better termed the flux density
of dry static energy
or enthalpy and characterizes
...
generateFigures = FALSE  ## set this TRUE to regenerate all figures;
                         ...
                         ## are used. 
opts_chunk$set(cache = !generateFigures)
...
Eq.\ \eqref{eq:C1} and
Eq.\ \eqref{eq:C2}. This demonstrates that the equations
Eq.\ \eqref{eq:Ts} and Eq.\ \eqref{eq:Tm} are a representation of the response
equivalent to Eq.\ \eqref{eq:FrieheKehlif} and to the equations used by
...
$R_{a}/(2C_{v})\approx1/5$, and $M$ small enough thatE the denominator
...
in Eq.\ \eqref{eq:QprimeOverQ} dominates, underestimation of the fluctuations
...
the first correction procedure discussed in 
...
Eq.\ \eqref{eq:tau1prime}.{\label{tab:Parameters}}}
...
The predicted time response from Eq.\ \eqref{eq:Tm} for assumed time constants
...
```
\caption{The weighted variance spectrum as a function of frequency $\nu$ for the recovery temperature measured by a Rosemount 102E4AL sensor. The spectra for the dynamic-heating term and for the calculated air temperature after this correction is applied are also shown.i\label{fig:S15}}
\end{figure}
...
Using the measurement of dynamic heating $Q$ as specified by Eq.\ \eqref{eq:DHterm}
...
\end{equation}
instead of Eq.\ \eqref{eq:recoveryTemperature}. The appropriate correction
...
\begin{figure}
```{r plotImpulse, include=TRUE, fig.height=4.5, out.width = "8.3cm"}
...
The results are shown in Fig.\ \ref{fig:SOCp1}.
This plot format
...
above \unit{1\,Hz} has been missed and therefore to judge if the frequency
...
if (generateFigures) {
  ...
  include_graphics('figure/fig12.pdf')
}
...
\begin{enumerate}
\item The differential equations Eq.\ \eqref{eq:Ts} and Eq.\ \eqref{eq:Tm}, with
...
Eq.\ \eqref{eq:Ts3}, this correction can be shown to be equivalent to
