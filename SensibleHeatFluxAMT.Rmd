...
the calculated air temperature becomes
\begin{equation}
T^{\prime}_{a}(t)=T_{m}(t)-Q^{\prime}(t){\label{eq:QprimeCorr}}
\end{equation}
instead of Eq.\ \eqref{eq:recoveryTemperature}. The filtered response
has been obtained from the transfer function in several equivalent ways, by
integration of the differential equations applied to $Q$, by Fourier transforms using the transfer function, and by applying a digital filter designed from the transfer function, all with essentially equivalent results. Results from all three approaches are presented here.
...
significant for the heated sensor, for which there are especially large fluctuations
...
plotted using a linear scale for the ordinate However, as the figure
...
negative values of the plotted magnitude. 
...
generated time series representing isotropic wind measurements with $-5/3$ slope of spectral variance vs.\ frequency as expected
for an inertial sub-range. The procedure was to generate
...
to reconstruct the simulated measurement series. The eddy dissipation
rate for the generated time series was\unit{10^{-3}\,m^{2}\,s^{-3}}.
<!-- \begin{equation} -->
...
the simulated temperature and updraft. The fluctuations in temperature
was generated similarly but scaled by a factor of \unit{0.2^{\circ}C\,m^{-1}\,s}, and then a correlation of 0.3 was introduced
between temperature and updraft fluctuations.
The correlation is thus the same at all frequencies
