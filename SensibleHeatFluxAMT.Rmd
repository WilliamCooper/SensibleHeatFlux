...
abstract: |
  ...
  especially measuring the flux density of
  sensible heat. This paper uses a new method
  ...
  improve measurements of the flux of sensible heat. Thermometers used
...
availability:
  ...
  codedata: |
    ...
    Laboratory, at this web site: \href{https://data.eol.ucar.edu}{https://data.eol.ucar.edu}.
...
acknowledgements: |
  ...
  Data provided by NCAR/EOL under the sponsorship of the National Science Foundation.
  ...
  The Copernicus template provided by Daniel Nüst, which uses the “rticles” package 
  for R (\citet{rticles}), was used with "RMarkdown" (\citet{XieEtAl2018RMarkdown}) to
  generate this paper.
...
The eddy-correlation measurement of the flux density of sensible heat ($F_{s}$) requires the measurement of the mean value of this expression (c.f., e.g., @hartmann201695):
\begin{equation}
...
(cf.\ their Eq.\ 2.23), who express dynamic heating $Q$ as the second equality in
...
this unheated sensor has been used widely as a fast-responding
...
To characterize the response of the unheated sensor, six
...
the one used here to represent the unheated sensor.
...
ParGV$tau1 <- ParGV$tau1 * (mean(MRHO, na.rm = TRUE) / 0.3) ^ 0.68
ParGV$tau2 <- ParGV$tau2 * (mean(MRHO, na.rm = TRUE) / 0.3) ^ 0.68
...
sensor was also onboard, so corrected measurements
...
be \unit{0.037\,s}, while the preceding equation predicts it should be
\unit{0.036}, so this provides some support for Eq.\ \eqref{eq:tau1prime}.
...
be to multiply $\tau_{1}$ by $(0.3/Z)^{0.68}$. (Having
...
Param1$tau1 <- Param1$tau1 * (mean(MRHO, na.rm = TRUE) / 0.3) ^ 0.68
# Param1$tau2 <- Param1$tau2 * (mean(MRHO, na.rm = TRUE) / 0.3) ^ 0.68
...
bestSH[2] <- bestSH[2] * (mean(MRHO, na.rm = TRUE) / 0.3) ^ 0.68
# bestSH[3] <- bestSH[3] * (mean(MRHO, na.rm = TRUE) / 0.3) ^ 0.68
...
\hline 
\hline 
unheated Rosemount 102E4AL on C-130 & `r round(Param1$a, 2)` & `r round(Param1$tau1, 3)` & `r round(Param1$tau2,2)`\tabularnewline
\hline 
...
\hline 
HARCO Model 100009-1 Deiced TAT& `r format(bestSH[1], digits=1, nsmall=1)` & `r format(bestSH[2], digits=2, nsmall=2)` & `r format(bestSH[3], digits=2, nsmall=2)`\tabularnewline
\hline 
\end{tabular}
\end{table}
...
next. One aspect of this correction needs emphasis, though. The correction
...
## A filter for dynamic heating\label{subsec:A-filter-for}
...
of the transfer function for the unheated sensor
and for the heated sensor, and the filter functions determined
from those impulse response functions.]{(a) The impulse response function found from the inverse Fourier
transforms of the transfer functions for the unheated and heated sensors, using the response parameters from Table\ \ref{tab:Parameters}. The impulse
response for the heated sensor is multiplied by 10.\protect \\
...
filter. The coefficients are multiplied by 10 for the heated sensor.{\label{fig:plotImpulse}}}
\end{figure}

Figure\ \ref{fig:plotImpulse}a shows the impulse function for the
unheated and heated sensors, and Fig.\ \ref{fig:plotImpulse}b
...
after filtering the dynamic-heating correction, for an unheated sensor and a heated sensor. ]{Comparison of the original measured temperature and the same temperature
after filtering the dynamic-heating correction:\protect \\
(left): An unheated sensor; (right): A heated sensor. {\label{fig:ATATF}}}
\end{figure}

For the unheated sensor, Fig.\ \ref{fig:ATATF}a
illustrates the removal of erroneous structure by filtering, and Fig.\ \ref{fig:ATATF}b
shows a similar example for the heated sensor. These examples
...
significant for the heated sensor, for which there are large fluctuations
...
of 100--500 times the boundary-layer height is needed. (See also \citet{lenschow1994long}.) The flight segments in these two
cases span about 80--150 times the boundary-layer height, so they are marginal by this criterion,
but the measurements still serve to illustrate the effect of the proposed correction. Uncertainties in this measurement
and common features of the atmospheric boundary layer are discussed by
\citet{lenschow95micro}. 
The flux estimates were then compared without correction and after correction
...
as it is without correction. A significant contribution to the error is the phase shift that causes the measured temperature to be shifted in phase relative to the updraft, as shown in Fig.~1. @LawsonRodi1992 estimated that,
...
of flux. Additional guidance regarding the expected shape of the cospectrum and the expected requirements for frequency measurements is provided by \citet{lenschow95micro}. The generalized cospectra provided there suggest that, for flight at \unit{130\,m\,s^{-1}}, frequencies up to about \unit{10\,Hz} or about \unit{1.3\,Hz} need to be measured at various altitudes below the top of a boundary layer, with the higher frequency limit applying to low-level flight at about \unit{30\,m} above the surface. Those results also suggest that, if the measurements are corrected as suggested here, reliable measurements of sensible-heat flux can be made with the unheated sensor.
...
as is characteristic of am unheated sensor. The simulated
...
sensor. That transfer function was shown to be consistent
...
to heated sensors, possibly indicating
...
sensor.
\item Once the transfer function for the unheated sensor has been
