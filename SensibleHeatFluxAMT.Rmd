...
---
\newpage
\introduction\runninglinenumbers
...
if(generateFigures) {  # suppressMessages(ggsave('figure/fig1.pdf', plot = g))
  cairo_pdf(filename='figure/fig1.pdf')  ## Using this to get symbol-font embedded
  print(g)
  invisible(dev.off())
}
...
to near or above $90^{\circ}$ for fast fluctuations. The amplitude and phase of the recovery temperature
...
between $T_m(t)$ and $Q$, so the excess variance above this frequency limit
is apparently not the result of fluctuations in dynamic heating.
...
g2 <- g2 + theme(axis.title.y = element_text(vjust = 2))
...
descent rate through the top of the marine boundary layer was approximately
...
same flight segment used for Fig.\ \ref{fig:S15}.
...
by filtering the dynamic-heating term.]{Variance spectra for a heated temperature sensor. 
...
brown lines, were calculated using dynamic-heating terms
...
by the transfer function, as shown by the solid brown lines.
