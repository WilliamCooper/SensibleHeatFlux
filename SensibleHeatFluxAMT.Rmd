...
the "recovery temperature," rather than the calculated air temperature
...
of the sensor will be represented here instead by two coupled differential
...
how dynamic heating affects the sensor. 
...
dynamic heating and true fluctuations in temperature. Thisfiltering
...
The filtered dynamic-heating adjustment will be denoted $Q^{\prime}$, so that
the calculated air temperature is estimated from
\begin{equation}
T_{a}(t)=T_{m}(t)-Q^{\prime}(t){\label{eq:QprimeCorr}}
\end{equation}
instead of Eq.\ \eqref{eq:recoveryTemperature}. The appropriate correction
has been obtained from the transfer function in several equivalent ways, by
integration of the differential equations applied to $Q$, by Fourier transforms using the transfer function, and by applying a digital filter, all with essentially equivalent results. Only the digital filter is discussed here because the others are straightforward, but comparisons from all three approaches are presented.
