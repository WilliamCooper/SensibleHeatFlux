...
abstract: |
  ...
  produced when air is compressed on approach to the sensor. The excess
  temperature above the ambient air temperature will be termed "dynamic
  heating." The magnitude
  of dynamic heating varies with airspeed, so the 
  transfer function that characterizes the sensor response 
  to turbulent fluctuations in dynamic heating can be measured. 
  ...
  parameters in those equations with low uncertainty. Examples
  ...
  temperature measurements from research aircraft, that results from failure
...
the measurand is the "recovery temperature," which
exceeds the air temperature by an amount termed here "dynamic heating" and
represented by the symbol $Q$.
The ambient
air temperature $T_{a}$ is related to the recovery temperature $T_{r}$ and
...
\begin{equation}\Theta(t)=A_{1}e^{-t/\tau_{1}}+A_{2}e^{-t/\tau_{2}}{\label{eq:FrieheKehlif}}\end{equation}
...
more complicated, as discussed in Appendix\ \ref{sec:Correcting-the-Temperature}. For a sinusoidal input these equations have analytic solutions after
...
Eq.\ \eqref{eq:Ts} and Eq.\ \eqref{eq:Tm} are consistent with the step-function response given by Eq.\ \eqref{eq:FrieheKehlif} and with the equations used by
...
RX1 <- CohP(DT1, 'TTRR', 'DH2', plotType = 'Ranadu', smoothBins = SB)
RX2 <- CohP(DT2, 'TTRR', 'DH2', plotType = 'Ranadu', smoothBins = SB)
RX3 <- CohP(DT3, 'TTRR', 'DH2', plotType = 'Ranadu', smoothBins = SB)
RX4 <- CohP(DT4, 'TTRR', 'DH2', plotType = 'Ranadu', smoothBins = SB)
RX5 <- CohP(DT5, 'TTRR', 'DH2', plotType = 'Ranadu', smoothBins = SB)
RX6 <- CohP(DT6, 'TTRR', 'DH2', plotType = 'Ranadu', smoothBins = SB)
phase <- rep(0, SB)
nphase <- rep(0, SB)
phase2 <- rep(0, SB)
ar <- rep(0, SB)
...
(referenced earlier in connection with the unheated probe on the GV)
were compiled
into one data set from the flight periods shown in Table\ 2. 
Corrected measurements from an unheated sensor were used as 
the reference against which to determine
the gain and phase of the transfer function of the heated probe.
...
The procedure used here starts from the Fourier
...
The measurement then is related to the true recovery temperature via
$\hat{T}_{m}(\nu)=H(\nu)\hat{T}_{r}(\nu)$ where $H(\nu)$ is the
transfer function.
The recovery temperature then can be obtained from the
...
This Fourier representation of the air temperature then can be multiplied
...
plotted using a linear prdinate scale. However, 
...
A similar time series for temperature was generated similarly but
