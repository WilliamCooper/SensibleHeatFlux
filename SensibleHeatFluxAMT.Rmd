...
abstract:
  ...
  sensors, with emphasis on the "fast" Rosemount unheated sensor.
  Airborne thermometers sense the
...
availability:
  codedata: |
    ...
    from the National Science Foundation (NSF) National Center for Atmospheric
    Research (NCAR), Earth Observing Laboratory (EOL), at the following web site: 
    ...
    via \LaTeX, using principles and techniques described by @Xie2015
    as implemented in the R package "knitr" [@Xie2021]. The
    ...
    language R [@Rlanguage] and were run within RStudio [@RStudio2021].
...
acknowledgements: |
  ...
  Data provided by NSF NCAR EOL. https://data.eol.ucar.edu/. 
  The data were collected using NSF’s Lower Atmosphere Observing Facilities, 
  which are managed and operated by NCAR’s Earth Observing Laboratory.
  Measurements used here (\citet{VOCALS2011}, \citet{SOCRATES2019}, \citet{CSET2017}) were collected in research projects
  (\citet{wood2011vamos},
  \citet{albrecht2019CSET}, \citet{mcfarquhar2021observations})
  that used
  the NSF/NCAR C130 (\citet{C130}) and GV (\citet{GV}) research aircraft. Project descriptions and additional
  information are available at the EOL data site: 
  \href{https://data.eol.ucar.edu}{https://data.eol.ucar.edu}.
  ...
  (\citet{RStudio2021})
  and knitr (\citet{Xie2015,Xie2021}). Data files in netCDF format
  have been read and written using the R package "ncdf4" (\citet{ncdf4}).
  ...
  (\citet{XieEtAl2018RMarkdown,Allaire2026}) to
  generate this paper. 
...
The eddy-correlation measurement of the flux density of sensible heat ($F_{s}$) is 
conventionally evaluated from the mean value of this expression (c.f., e.g., @hartmann201695):
\begin{equation}
...
mean and angle brackets denote an ensemble average. 
This measurement thus requires
that temperature be measured with sufficient response
to resolve the eddy sizes that make significant contributions to the flux.
(Additional
information regarding measuring fluxes by eddy correlation is contained in, e.g., \citet{lenschow95micro}.) 

@FrieheKhelif1992 suggested that 4--5\unit{\,Hz} is "just
...
# setwd('~/RStudio/SensibleHeatFluxAMT/CopernicusFiles')
source('chunks/initialize.R')
source('chunks/theme_WAC.R') ## modified to make background more gray, less pink
# source('chunks/VSpec.R')
opts_chunk$set(fig.height = 3.6, digits = 3)
...
of many standard temperature sensors used on research aircraft. The suggested explanation [@NCAR_OpenSky_TECH-NOTE-000-000-000-064]
is that heat is transferred to the sensing wire of those sensors
...
by @FrieheKhelif1992 for an unheated Rosemount 102E4AL sensor were \{0.65, 0.35, \unit{0.09\,s}, \unit{0.5\,s}\}.

This work, following the approach of @PayneEtAl1994, represents the time
response of the sensor by two coupled differential equations,
...
the fraction of the heat transferred by the air, while the fraction $(1-a)$ is
...
These equations have analytic solutions that are sinusoidal functions if the measurand is sinusoidal.
For example, if the
...
\begin{figure}
```{r LTsolution, include = TRUE, fig.height = 3.7, out.width = "8.3cm"}
...
g <-
  ggplotWAC(
    dfAP,
    panels = 2,
    labelP = c(bquote(' ' ~ 'Amplitude Ratio'), Panel2Label),
...
g <- g + theme_WAC(1) + theme(legend.position = c(0.3, 0.65), legend.text=element_text(size=14))
if(generateFigures || !(file.exists('figure/fig1.pdf'))) {  # suppressMessages(ggsave('figure/fig1.pdf', plot = g))
  cairo_pdf(filename='figure/fig1.pdf')  ## Using this to get symbol-font embedded, for \nu
  print(g)
  invisible(dev.off())
}
...
for the derivatives (Eqs.\ \eqref{eq:Ts} and \eqref{eq:Tm}) or by dividing the 
...
The evaluation of the time response that follows uses the fluctuations in dynamic
...
the recovery temperature expressed in absolute units, $M$
...
of the response relative to the imposed signal will vary,
from near $0^{\circ}$ for fluctuations slow compared to sensor response
to near or above $90^{\circ}$ for fast fluctuations. The amplitude and phase 
of the measured recovery temperature
relative to the forcing from dynamic heating therefore can be used as sensitive
...
aircraft, a Gulfstream V (hereafter, GV) and a Hercules C-130. Brief descriptions of these aircraft are available at these sources: \citet{GV}, \citet{C130}. The
...
evidence for delayed response of standard sensors.
The unheated sensor referenced above and cited in these studies
has been used widely as a fast-responding
...
and @mcfarquhar2021observations respectively, each included low-level
flight segments in the boundary layer over the Pacific Ocean.
Details and specific references to the data sets are included in the
section on "Code and data availability" near the end of this paper.
...
estimated fluctuations determined from Eq.\ \eqref{eq:DHterm}. 
...
<!-- differs from $\delta T_{r}/T_{r}$.   -->
For a representative low-level 
...
the variance of the second term
on the right side of Eq.\ \eqref{eq:QprimeOverQ} is more
...
the transfer function was determined, $T_{r}(t)$ was calculated using
the correction procedure discussed in 
Appendix\ \ref{sec:Correcting-the-Temperature}.
This corrected estimate of $T_{r}(t)$ 
led to only a minor change in the fitted values of the parameters, and
the estimate was stable after only one additional iteration.
...
reviseDH <-
  function (.data, P, AR, alphaR) {
    ...
    QF <- as.vector(signal::filter(AR, .data$Q))
    QF <- ShiftInTime(QF, .shift = -Lshift * 40, .rate = 25)
    return (.data$ATX + alphaR * (.data$Q - QF))
  }
if (file.exists ('./DVOCALS.Rdata')) {
  ...
  D <- getNetCDF(
    ...
    c(
      ...
      'PSXC',
      'QCXC',
      'QCFC'  ## strangely, QCRC was used as primary -- usually not the best
    )
  )
  # D$QCXC <- D$QCFC
  D$DH <- D$TASX ^ 2 / 2010
  ## For use later, "filter" the dynamic-heating term and revise the temperature:
  alphaR <- attr(D[, 'ATRR'], 'RecoveryFactor') # 0.95 used; not the best
  D$AT <- reviseDH(D, Param1, AR, alphaR) # Param1 and AR for unheated, C-130
...
To characterize the frequency-domain transfer function of an
unheated temperature sensor, six
...
listed in Table\ 1. None of these flight segments included
passage through cloud, to avoid possible errors in temperature
measurements that arise when cloud water contacts the sensor.
For each flight segment, the phase and amplitude
ratio between the measured recovery temperature
...
average phase are shown in Fig.\ \ref{fig:Vphase}b. The theoretical
...
\begin{figure}
```{r Vphase, include=TRUE, echo=FALSE, fig.height = 4.51, out.width = "12cm"}
...
g <-
  g + geom_path(data = dfAP,
                ...
                col = 'forestgreen',
                linewidth = 1.5)
g <- g + annotate('text', x = 10, y = 0, label = '(b)', size = 8)
g1 <- g + theme_WAC(1)
frq <- seq(0.01, 25, by = 0.01)
...
g <- g + annotate('text', x = 10, y = 1.05, label = '(a)', size = 8)
...
if (generateFigures || !(file.exists('figure/fig2.pdf'))) {
  cairo_pdf('figure/fig2.pdf')
  suppressWarnings(print(g2, vp = vp2))
  suppressWarnings(print(g1, vp = vp1))
  invisible(dev.off())
} 
include_graphics('figure/fig2.pdf')
...
is shown in Fig.\ \ref{fig:Vphase}a. It is useful to consider both
...
the set of favored parameters, Fig.\ \ref{fig:Vphase}a shows the
...
Above \unit{3\,Hz} the prediction is much too low. This could arise because
there is spurious variance in the measurement $T_m(t)$ not caused by
dynamic heating or because 
the measurement of dynamic heating is attenuated at high frequency. The
variance spectra (not shown) suggest both may contribute to the discrepancy,
but excess variance in the measured recovery temperature appears to be
the more significant problem. The variance spectrum of $T_m(t)$ is reduced
substantially above \unit{3\,Hz} if it is multiplied by the squared coherency
between $T_m(t)$ and $Q$, so most of the excess variance above this frequency
is apparently not attributable to fluctuations in dynamic heating.
Exclusion of these measurements from the fit is therefore reasonable.
...
the parameters from this first fit to predict the actual
recovery temperature $T_{r}(t)$. The calculation
of phase and amplitude was then repeated after recalculating $Q$ using 
Eq.\ \eqref{eq:DHterm}
and that new prediction of $T_{r}(t)$, and the results were fitted again
...
DSX <- getNetCDF(
  file.path(DataDirectory(), 'SOCRATES/SOCRATESrf15h.nc'),
...
DSX$RT <- DSX$RTF1
DCX$RT <- DCX$RTF1
## Correct RT for time response: (optional)
## (This doesn't work; see later discussion. Erroneous Q still in
##  RT is amplified.)
if(FALSE) {
  a <- Param1$a
  tau1 <- Param1$tau1
  ...
  # DCX$DTMDT <- c(0, diff(DCX$RT, 2), 0) * Rate / 2
...
}  ## end of optional correction to RT for time response
...
## These next three lines aren't used; they are left over from earlier testing:
...
phaseSF <- rep(0, SB)  ## initialize sums
nphaseSF <- rep(0, SB)
phase2SF <- rep(0, SB)
arSF <- rep(0, SB)
ar2SF <- rep(0, SB)
...
for (RX in list(RXA, RXB, RXC, RXD, RXE)) {
  for (i in 2:nrow(RX)) {  ## start at 2 because binStats gives NA for 1
    ...
    arSF[RX$BIN[i]] <-
      arSF[RX$BIN[i]] + sqrt(RX$P.spec...1.[i] / RX$P.spec...2.[i])
    ar2SF[RX$BIN[i]] <-
      ar2SF[RX$BIN[i]] + (RX$P.spec...1.[i] / RX$P.spec...2.[i])
    narSF[RX$BIN[i]] <- narSF[RX$BIN[i]] + 1
  }
}
...
Fits using the three-parameter representation of the transfer
function were unsatisfactory for the heated sensor, 
...
or parameters determining the transfer function for the heated sensor.
...
\begin{figure}
```{r HARCOSpec, include = TRUE, out.height = '5cm', out.width = "8.3cm"}
...
gH <- DCX %>% selectTime(175200, 180200) %>% 
  select(Time, TASX, RT, RTH1) %>%
    ...
    theme_WAC(1) + 
    theme(legend.position=c(0.7, 0.9), legend.text=element_text(size=14))
if (generateFigures || !(file.exists('figure/fig3.pdf'))) {
  cairo_pdf('figure/fig3.pdf')
  print(gH)
  invisible(dev.off())
} 
...
```
\caption{Spectral variance $P(\nu)$ weighted by frequency ($\nu$) for the recovery temperature sensed by a heated and an unheated temperature sensor. The background includes orange dotted lines denoting $-2/3$ slope and the wavelength scale is determined from the average airspeed.\label{fig:HARCOSpec}}
\end{figure}
...
\begin{figure}
```{r GVHARCO, include = TRUE, fig.height = 4.51, out.width = "12cm"}
...
g1 <- g + theme_WAC(1) +theme(legend.position='none')
...
g2 <- g + 
      ylim(0, 1.05) + 
      theme_WAC(1) + theme(legend.position = c(0.75, 0.75),
                               legend.direction = 'vertical')
## small adjustment for alignment
g2 <- g2 + 
      theme(plot.margin = unit(c(0.3, 0.3, 1.1, 1.8), "lines"))
g2 <- g2 + theme(axis.title.y = element_text(vjust = 2), legend.text=element_text(size=14))
g1 <- g1 + theme(axis.title.y = element_text(vjust = 0), legend.text=element_text(size=14))
if (generateFigures) grid.newpage()
vp2 <-
  viewport(
    y = unit(0.59, 'npc'),
    height = unit(0.48, 'npc'),
    layout.pos.row = 2
  )
vp1 <-
  viewport(
    y = unit(0.25, 'npc'),
    height = unit(0.45, 'npc'),
    layout.pos.row = 1
  )
if(generateFigures || !(file.exists('figure/fig4.pdf'))) {
  cairo_pdf('figure/fig4.pdf')
  print(g2, vp = vp2)
  print(g1, vp = vp1)
  invisible(dev.off())
}
...
as the blue lines labelled "3-par" in that figure. The fitted values
...
the air to the sensing wire, but instead all is transferred from the
support to the wire, while the support has a relatively slow characteristic response. 
...
of the measurements, as shown by the dashed green lines labelled "lfit".
...
the three-parameter fit, although the fit is not valid above about \unit{2\,Hz}
because those values were not constrained by the measurements.
<!-- For completeness, replicating the value at \unit{2\,Hz} to higher frequencies -->
<!-- may be reasonable because there is so little variance measured -->
...
<!-- <!-- This section has been modified significantly to relate to the recovery -->
<!-- <!-- temperature rather than the ambient temperature and to correct an error -->
<!-- <!-- in the calculation. -->
<!-- The section has now been omitted because it no longer provides  -->
<!-- useful support for the transfer function as determined here. -->
<!-- Furthermore, the top of the BL is cloudy, so I worry about -->
<!-- the complication that sensor wetting may introduce. -->
<!-- \begin{figure} -->
<!-- ```{r tfit, include=TRUE, echo=FALSE, fig.height=3.5, out.width = "8.3cm"} -->

<!-- ## The code below was used to generate fig5.pdf: -->
<!-- if (generateFigures || !(file.exists('figure/fig5.pdf'))) { -->
<!--   cairo_pdf('figure/fig5.pdf') -->
<!--   Data <- getNetCDF(file.path(DataDirectory(), 'VOCALS/VOCALSrf03h.nc'), -->
<!--                     c('PALT', 'GGALT_NTL', 'ATX', 'ATRL', -->
<!--                       'ATRR', 'ATWH', 'DPXC', 'PSXC', -->
<!--                       'QCXC', 'TTRR'), -->
<!--                     81350, 81357)  ## the time interval (up to 81358) -->
<!--   ## Get Z for scaling the time constants: -->
<!--   rhozero <- 1013.25 * 100 / (287.05 * 288.15) -->
<!--   Z <- MachNumber(Data$PSXC, Data$QCXC) * 100 * Data$PSXC / -->
<!--     (287.05 * (273.15 + Data$ATRR)) / rhozero -->
<!--   tau1 <- Param1$tau1 * (0.3 / mean(Z, na.rm = TRUE) ) ^ (0.68) -->
<!--   tau2 <- Param1$tau2 -->
<!--   a <- Param1$a -->
<!--   nf <- 50  ## subdivision factor to use for integration -->
<!--   TTOP <- 20.2  ## recovery T above and below the inversion -->
<!--   TBOT <- 12.4 -->
<!--   ## Adjustment to adiabatic stratification for approx ratio, recovery to ambient T: -->
<!--   Slope <- 6.5 / 1000 * (mean(Data$TTRR) + 283.15) / (mean(Data$ATRR) + 273.15) -->
<!--   dt <- 1 / (nf * 25)  ## integration interval -->
<!--   ## Plot recovery temperature -->
<!--   Data %>%  -->
<!--     select(Time, TTRR) %>%  -->
<!--     plotWAC(type='b', ylab = bquote('recovery temperature ['*degree*'C]')) -->
<!--   ## Estimate true recovery temperature: -->
<!--   Data$T <- TBOT - (0:(nrow(Data)-1)) * 5 / 25 * Slope  ## 10C per km, 5 m/s descent  -->
<!--   Data$T[1:10] <- TTOP  ## force above-inversion value -->
<!--   ## Get fine-scale true recovery temperature for integration -->
<!--   yy <- TBOT - (0:(nf*nrow(Data)-1)) * 5 / (nf*25) * Slope  -->
<!--   yy[1:(10*nf)] <- TTOP  ## force above-inversion value -->
<!--   lineWAC(Data$Time[11:200], Data$T[11:200], col='black', lty=3) -->
<!--   lineWAC(c(Data$Time[11], Data$Time[10]), c(Data$T[11], Data$T[10]), col = 'black', lty = 3) -->
<!--   xx <- (1:(nf*nrow(Data)) - 1) * dt + Data$Time[1] -->
<!--   # calculate the response, given tau: -->
<!--   FF <- 1  ## fudge factor for exploring effects; try 2.5 -->
<!--   tau1 <- FF * tau1 -->
<!--   tau2 <- FF * tau2 -->
<!--   ym <- yy -->
<!--   yr <- yy -->
<!--   for (i in 2:(nrow(Data)*nf)) { -->
<!--     yr[i] <- yr[i - 1] + (yy[i] - yr[i - 1]) * dt / tau2 -->
<!--     ym[i] <- ym[i - 1] + (a * yy[i] + (1 - a) * yr[i] - ym[i - 1]) * dt / tau1 -->
<!--   } -->
<!--   lineWAC(xx, ym, col = 'magenta', lwd = 3, lty = 1) -->
<!--   lineWAC(xx, yr, col = 'brown',   lwd = 3, lty = 2) -->
<!--   legend('topright', legend=c('measured', 'predicted', 'support', 'if discrete'), -->
<!--          lwd=c(2, 2, 2, 2), lty=c(1, 1, 2, 3), -->
<!--          col=c('blue', 'magenta', 'brown', 'black'), -->
<!--          pch = c(19, NA, NA, NA)) -->
...
<!-- ``` -->
<!-- \caption{(blue dots): Recovery temperature measured at \unit{25\,Hz} during descent through an inversion capping the marine boundary layer, from VOCALS flight\ 3. The dotted black line shows the estimated true recovery temperature, featuring a sharp change at the top of the -->
<!-- boundary layer and a dry-adiabatic lapse rate in the marine boundary layer. The  prediction using the parameters listed in the text is shown as the magenta line, and the dashed brown line is the calculated temperature of the support that contacts the sensing wire.\label{fig:tfit}} -->
<!-- \end{figure} -->

<!-- Most prior studies have used sharp temperature changes in the -->
...
<!-- discrete transitions. The best case found after a search of candidate passes through -->
<!-- inversions in the VOCALS project is shown in  -->
<!-- Figure\ \ref{fig:tfit}, from VOCALS flight 3, where the aircraft -->
<!-- descended through the top of the marine boundary layer at approximately -->
<!-- \unit{5\,m\,s^{-1}}. -->
<!-- The assumed measurand history, for an assumed discrete discontinuity at the inversion -->
<!-- and a well-mixed cloudy boundary layer below, is that shown by the dotted black line. -->
...
<!-- The observations initially change as fast as the predicted response, but after about -->
<!-- \unit{0.2\,s} the measurements change significantly slower than predicted. -->
<!-- The predictions and measurements only agree if the time constants are increased -->
<!-- by about a factor of 2.5. It appears that, like all other boundary-layer tops -->
<!-- observed in this project, the actual structure is modified by mixing at the cloud top and -->
<!-- is not the discrete discontinuity assumed in these calculations. Nevertheless, -->
<!-- the observations provide some support for the conclusion that the time response is, -->
<!-- within about a factor of 2.5, about as  -->
<!-- estimated from the transfer function determined from fits to -->
...
<!-- to compensate for the time response of the sensors.  -->
<!-- In Section\ \ref{sec:Correcting-for-Dynamic} the -->
<!-- transfer functions are used to assess how sensors respond to dynamic -->
<!-- heating and to develop appropriate correction schemes. Then  -->
<!-- Section\ \ref{sec:flux-density} uses the transfer functions to develop  -->
<!-- improved measurements of the flux of sensible heat. -->
...
cannot respond to the measured fluctuations in dynamic heating, this procedure
...
<!-- \begin{figure} -->
```{r S15, include=TRUE, fig.height = 3.7, out.width = "8.3cm"}

DS <- getNetCDF(
  file.path(DataDirectory(), 'SOCRATES/SOCRATESrf15h.nc'),
            standardVariables(c('ATF1', 'ATH1', 'UXC')),
            60000,
            61500)
...
DS$RTH1 <- DS$ATH1 + rf * DS$TASX^2 / 2010
...
<!-- if (generateFigures || !(file.exists('figure/fig5.pdf'))) { -->
<!--   cairo_pdf('figure/fig5.pdf') -->
<!--   ## I have changed this plot because the former plot had -->
<!--   ## spurious variance in Q from line resonance on the GV. -->
<!--   ## It seemed better to use a C-130 example to avoid the -->
<!--   ## distraction of that noise. -->
<!--   ggg <- DT5 %>% Rmutate(Tr = TTRR, Ta = ATRR, Q = DH) %>% -->
<!--        select(Time, TASX, Tr, Ta, Q) %>% -->
<!--        VSpecC(VLabel = c('recovery temp.', 'air temp.', 'dynamic heating'), -->
<!--              spans = 199, xlim = xlim, ylim=c(1.e-5, 0.03)) + -->
<!--        suppressWarnings( -->
...
<!--       legend.background=element_rect(colour = 'black', liinewidth = 0.3, -->
<!--                                      fill = "ivory"), -->
<!--       legend.direction = "vertical",  -->
...
<!-- ``` -->
<!-- \caption{The weighted variance spectrum as a function of frequency $\nu$ for the recovery temperature measured by an unheated sensor. The spectra for the dynamic-heating term and for the calculated air temperature after this correction is applied are also shown. The wavelength scale is determined from the average airspeed, and the background includes orange dotted lines denoting $-2/3$ slope. Data from VOCALS flight 3, 12:43:00 to 12:53:00 UTC.\label{fig:S15}} -->
<!-- \end{figure} -->

Figure\ \ref{fig:f5} illustrates the problem. The measurements are
from a low-level flight segment over the ocean where other indications
are reasonably consistent with an inertial subrange. 
The heated sensor gives the measurement conventionally assumed to be the
recovery temperature, as shown by the dashed blue line. The slow sensor does
not respond to the high-frequency changes in dynamic heating, and as
a result subtraction of the dynamic-heating term to obtain the air
temperature (shown as the thick red line)
introduces erroneous high-frequency fluctuations in the result
that are opposite to the fluctuations in dynamic heating. These fluctuations
are not real; they occur because the sensor does not respond to the fluctuations
in dynamic heating so subtraction of those fluctuations introduces erroneous
fluctuations in the calculated air temperature. A similar plot for the
unheated sensor indicates a similar effect, although it is reduced in
magnitude because that sensor has much faster response.
\begin{figure}
```{r f5, include=TRUE, fig.height = 3.7, out.width = "8.3cm"}

if (generateFigures || !(file.exists('figure/fig5.pdf'))) {
  DP5 <- DT5 %>% selectTime(125000, 125005) 
  DP5 <- transferAttributes(DT5, DP5)
  DP5$Ta <- DP5$ATWH - mean(DP5$ATWH, na.rm=TRUE)
  DP5$Tm <- DP5$TTWH - mean(DP5$TTWH, na.rm=TRUE)
  DP5$Q <- DP5$DH - mean(DP5$DH, na.rm=TRUE)
  ggg5 <- DP5 %>%  select(Time, Tm, Q, Ta) %>% 
    ggplotWAC(lty=c(6,1,1), lwd=c(2,1,2)) + theme(legend.text = element_text(size=14)) +
    xlab('seconds past 12:50:00 UTC') +
    ylab(bquote('Tm ['*degree*'C]'))
  cairo_pdf('figure/fig5.pdf')
  print(ggg5)
  invisible(dev.off())
}
...
The new approach followed here is to filter the dynamic-heating term to match the response of the sensor, so that only the actual sensor response to dynamic heating is subtracted from the measurement. This is made possible by the assumed linearity in response of the sensor. The proposed filtering removes a significant source of erroneous fluctuations
present in many measurements of temperature made from research aircraft.
<!-- However, the variance spectrum for -->
<!-- the estimated ambient temperature, after correction by subtracting the contribution -->
<!-- from dynamic heating, appears to have a substantial amount -->
<!-- of high-frequency contamination. This matches the high-frequency portion -->
<!-- of the spectrum for dynamic heating. The spectrum for the recovery temperature -->
<!-- should exceed that of the ambient -->
<!-- temperature (as is the case for measurements below about \unit{1\,Hz}) so that the subtraction -->
...
The filtered response to dynamic heating expected for the temperature sensor
has been obtained from the transfer function in three ways: by
integration of the differential equations (Eqs.\ \eqref{eq:Ts} and \eqref{eq:Tm}) applied to $Q$ instead of $T_r$, by Fourier transforms using the transfer function, and by applying a digital filter designed from the transfer function. The digital filter was designed using standard methods as outlined in, e.g., \citet{Press:1992:NRC:148286}; details and the coefficients used are
available in the Workflow document that accompanies this paper. All three methods produce almost equivalent results. 
The filtered dynamic-heating adjustment will be denoted $Q^{\prime}$, and the calculated 
air temperature $T_a^\prime$ then becomes
\begin{equation}
...
\begin{figure}
```{r Integration, include=TRUE, echo=FALSE, out.height = "5cm", out.width = "12cm", messages = FALSE}
...
g <- g + theme_WAC(1) +
  ...
  guides(linewidth = guide_legend(reverse = TRUE, 'variable:')) +
...
g2 <- DS %>% select(Time, TASX, AT, RT, ATF1) %>%
  ...
  guides(linewidth = guide_legend(reverse = TRUE, 'variable:')) +
  ylab(bquote('spectral variance ' * nu * 'P(' * nu * ')'))
if (generateFigures || !(file.exists('figure/fig6.pdf'))) {
  cairo_pdf('figure/fig6.pdf')
  print(g2, vp = vp2)
  print(g1, vp = vp1)
  invisible(dev.off())
}
...
The result for the latter (dashed greenline) is so close to that for "FFT" that it
is obscured in this plot.\protect \\ 
(b): The recovery temperature sensed by the sensor without correction and the ambient temperature
calculated using the digitally filtered dynamic-heating term (but without correction for the sensor response time). The original
variable for ambient temperature based on standard processing 
(labelled "with std Q") is also shown.{\label{fig:Integration}}}
...
substantial high-frequency variance in excess of expectations for
an inertial subrange. Lenschow (in @cooper2016ncartn) argued that this
is the result of resonance in the sampling lines, so it does not
reflect a true influence on the recovery temperature.
The modified variance spectrum obtained
...
digital filter representing the transfer function, labelled "filter", or
after Fourier transformation, labelled "FFT", are mostly overlapping.
These latter corrected estimates
...
\unit{25\,Hz} smoothing, integrated, and then resampled to obtain \unit{25\,Hz} 
measurements, so the high-frequency discrepancy appears
...
temperature sensor. The digital filter has the advantage that it can be applied
to an entire flight and avoids the need to segment and patch results from the
Fourier transform. 

A revised estimate of the ambient air temperature was then calculated using
Eq.\ \eqref{eq:QprimeCorr} and the filtered dynamic-heating term $Q^{\prime}$.
...
using the standard dynamic-heating term. Even at \unit{1\,Hz}, filtering removes a significant and erroneous contribution to the variance in the
measured ambient temperature.

```{r CSETX, include=TRUE, echo=FALSE}

# DWECAN <- getNetCDF(
#   file.path(DataDirectory(), 'WECAN/WECANrf17h.nc'), 
#             standardVariables(c('RTH1',
...
## Delete the above; can't use preliminary WE-CAN files
# Repeat for CSET rf04h
DCSET <- getNetCDF(
  file.path(DataDirectory(), 'CSET/CSETrf05h.nc'), 
            standardVariables(c('RTH1',
                    'ATH1', 'RTF1', 'ATF1')), 175200, 180200) # 182600,182900)
DCSET$Q <- DCSET$TASX^2 / 2010
SB <- 50
Z <- capture.output (
       Vr <- SmoothInterp(detrend (DCSET[, c('Time', 'RTH1')]), .Length = 0))
Z <- capture.output (
       VrC <- SmoothInterp(detrend (DCSET[, c('Time', 'Q')]), .Length = 0))
Rate <- attr(DCSET, 'Rate')
vcv <- cbind(ts(Vr, frequency = Rate), ts(VrC, frequency = Rate))
P <- spec.pgram(vcv, detrend = FALSE, fast = TRUE, plot = FALSE, spans = 25)
dfH <- data.frame(P$coh, log(P$freq))
pfH <- binStats (dfH, bins = SB)
RXH <- CohP(DCSET, 'RTH1', 'Q', plotType = 'Ranadu', smoothBins = SB)
phaseH <- rep(0, SB)
nphaseH <- rep(0, SB)
phase2H <- rep(0, SB)
arH <- rep(0, SB)
...
frequencies above about \unit{0.1\,Hz}.

\begin{figure}
```{r HarcoQ, include=TRUE, echo=FALSE, out.height = "5cm", out.width = "12cm", messages = FALSE}

a <- 0.733
tau1 <- 0.299
tau2 <- 0.433
DCSET$RT <- SmoothInterp(DCSET$RTH1, .Length = 0)
DCSET$TsQ <- DCSET$RT
DCSET$Q <-
  SmoothInterp(DCSET$Q, .Length = 0)  ## interpolate for missing values
Rate <- attr (DCSET, 'Rate')
rhozero <- 1013.25 * 100 / (287.05 * 288.15)
MRHO <- MachNumber(DCSET$PSXC, DCSET$QCXC) * 
        DCSET$PSXC * 100 / (287.05 * (273.15 + DCSET$ATX)) / rhozero
...
DCSET$Qp <- DCSET$Q
fS <- function(y, i) {
  (DCSET$Q[i] - y) / (Rate * tau2[i])
}
fM <- function (y, i) {
  (a * DCSET$Q[i] + (1 - a) * DCSET$TsQ[i] - y) / (Rate * tau1[i])
}
DCSET$TsQ <- rk4.integrate (fS, DCSET$Q[1], 1:nrow(DCSET))
DCSET$Qp <- rk4.integrate (fM, DCSET$Q[1], 1:nrow(DCSET))
DCSET$QF <- as.vector(signal::filter(ARH, DCSET$Q))
DCSET$QF <- ShiftInTime(DCSET$QF, .shift = -LshiftH * 40, .rate = 25)
g1 <- DCSET %>% select(Time, TASX, Q, QF, Qp) %>%
  VSpecC(
  ...
  theme_WAC(1) + theme(legend.text = element_text(size = 14)) +
  theme(
  ...
  # guides(colour = guide_legend('variable:'),
  #        linetype = guide_legend('variable:', 
  #                   override.aes = list(linewidth = c(1, 2.5, 0.7)))) +
  ylab(bquote('spectral variance ' * nu * 'P(' * nu * ')'))
grid.newpage()
vp2 <-
  viewport(
...
DCSET$AT <- DCSET$RT - DCSET$QF
# save(DWECAN, file = 'DWECAN.Rdata')
g2 <- DCSET %>% select(Time, TASX, AT, RTH1, ATH1) %>%
  VSpecC(
    VLabel = c(
      'air temp. with filt. Q',
      'msrd recovery temp.',
  ...
  guides(colour = guide_legend('variable:'),
         linetype = guide_legend('variable:'),
         linewidth = guide_legend('variable:')) +
  ylab(bquote('spectral variance ' * nu * 'P(' * nu * ')'))

generateSave <- generateFigures
generateFigures <- TRUE
if (generateFigures) {
  cairo_pdf('figure/fig7.pdf')
  suppressWarnings(print(g2, vp = vp2))
  print(g1, vp = vp1)
  invisible(dev.off())
}
...
by filtering the dynamic-heating term.]{Variance spectra for the dynamic-heating correction applied to a heated temperature sensor. (Data from
the CSET project, flight 5, 18:04:00 to 18:35:00 UTC.) 
...
(b): Variance spectra for the air temperature as modified by filtering the dynamic-heating
...
spectra properly represent how the temperature sensor responds to dynamic-heating
fluctuations, filtering $Q$ removes substantial erroneous
variability otherwise present in the calculated air temperature.

\begin{figure}
```{r ATATF, include=TRUE, echo=FALSE, out.height="5cm",  out.width = "12cm", messages = FALSE}
generateSave <- generateFigures
# generateFigures <- TRUE  ## temporary; reset below
if (file.exists ('./DVOCALS.Rdata')) {
  load (file = './DVOCALS.Rdata')
  DVOCALS <- D
  DVOCALS$MACHX <- MachNumber(DVOCALS$PSXC, DVOCALS$QCXC)
  DVOCALS$ATC <- DVOCALS$AT
  DVOCALS$original_Q <- DVOCALS$ATRR
  DVOCALS$filtered_Q <- DVOCALS$ATC
  ga <- DVOCALS %>% select(Time, original_Q, filtered_Q) %>%
    selectTime(80100, 80104) %>%
    ggplotWAC(col = c('darkorange', 'black'), lwd = c(1, 2), ltyp = c(1,4)) +
    theme_WAC(1) +
    ylab(bquote('temperature [' * degree * 'C]')) +
    xlab('time after 8:01:00 UTC [s]') +
    ...
    guides(
      ...
      linewidth = guide_legend("air T:"),
      linetype = guide_legend("air T:")
    )
}
# generateSave <- generateFigures
# # generateFigures <- TRUE  ## temporary; reset below
# SV <- standardVariables()
# DCSET <- getNetCDF(
#   file.path(DataDirectory(), 'CSET/CSETrf12hT.nc'),
#       c(SV, 'ATF1', 'ATH1', 'RTF1', 'RTH1',  'ATF1C', 'ATH1C')  
# )  ## strangely, QCRC was used as primary -- usually not the best
# recoveryFactor <- 0.9849
# DCSET$Q <- recoveryFactor * DCSET$TASX^2 / 2010
# DCSET$Q <- SmoothInterp(DCSET$Q, .Length=0)
# Rate <- attr (DCSET, 'Rate')
# rhozero <- 1013.25 * 100 / (287.05 * 288.15)
# MRHO <- MachNumber(DCSET$PSXC, DCSET$QCXC) * 
#         DCSET$PSXC * 100 / (287.05 * (273.15 + DCSET$ATX)) / rhozero
...
# a <- ParamSF$a
# ## Note that these are variable:
# tau1 <- ParamSF$tau1 * (MRHO / 0.3) ^ 0.68
# tau2 <- rep(ParamSF$tau2, length(MRHO))
# # tau2 <- ParamSH$tau2 * (MRHO / 0.3) ^ 0.68
# Rate <- attr (DCSET, 'Rate')
# DCSET$TsQ <- DCSET$Q
# DCSET$Qp <- DCSET$Q
...
# DCSET$TsQ <- rk4.integrate (fS, DCSET$Q[1], 1:nrow(DCSET))
# DCSET$Qp <- rk4.integrate (fM, DCSET$Q[1], 1:nrow(DCSET))
# DCSET$QF <- as.vector(signal::filter(AR1, DCSET$Q)) # AR for unheated
# DCSET$QF <- ShiftInTime(DCSET$QF, .shift = -LshiftH * 40, .rate = 25)
# # if (file.exists ('./DVOCALS.Rdata')) {
# #   load (file = './DVOCALS.Rdata') # loads D
# # DVOCALS <- D
#   # DVOCALS$ATC <- DVOCALS$AT
#   DCSET$ATF1F <- DCSET$ATF1 + DCSET$Q - DCSET$QF
#   DCSET$original <- DCSET$ATF1
#   DCSET$filtered <- DCSET$ATF1F
#   startTime <- 174000; endTime <- 175000
#   ga <- DCSET %>% select(Time, original, filtered) %>%
#     # selectTime(startTime, endTime) %>%
#     selectTime(180000, 180004) %>%
#     ggplotWAC(col = c('darkorange', 'black'), lty=c(1,1), lwd = c(2.5, 1.0)) +
#     theme_WAC(1) +
#     ylab(bquote('temperature [' * degree * 'C]')) +
#     xlab('time after 18:00:00 UTC [s]') +
#     theme(legend.direction = "vertical",
#           legend.position = c(0.5, 0.25),
#           legend.key.width = unit(1, 'lines'),
#           legend.key.height = unit(0.5, 'lines'),
...
#     )
  ...
  if (generateFigures || !(file.exists('figure/fig8.pdf'))) {
    cairo_pdf('figure/fig8.pdf')
    print (ga + annotate(
      'text',
...
# }
DVOCALS$MACHX <- SmoothInterp (DVOCALS$MACHX, .Length = 0)
DVOCALS$ATWH <- SmoothInterp (DVOCALS$ATWH, .Length = 0)
alphaR2 <- 0.985 # attr(DVOCALS$ATWH, 'RecoveryFactor')
# alphaR2 <-
#   0.988 + 0.053 * log10(DVOCALS$MACHX) +
#   0.090 * (log10(DVOCALS$MACHX)) ^ 2 +
#   0.091 * (log10(DVOCALS$MACHX)) ^ 3
DVOCALS$Q <- alphaR2 * DVOCALS$TASX^2 / 2010
# DVOCALS$AT <- reviseDH(DVOCALS, ParamSH, ARH, alphaR2)
DVOCALS$QF <- as.vector(signal::filter(ARH, DVOCALS$Q))
DVOCALS$QF <- ShiftInTime(DVOCALS$QF, .shift = -18*Lshift1*40, .rate = 25)
DVOCALS$AT <- DVOCALS$ATWH - (DVOCALS$QF - DVOCALS$Q)
DVOCALS$filtered_Q <- DVOCALS$AT
DVOCALS$original_Q <- DVOCALS$ATWH
gb <- DVOCALS %>% select(Time, original_Q, filtered_Q) %>%
  selectTime(80100,80106) %>%
  ggplotWAC(col = c('darkorange', 'black'), lwd = c(1, 2)) +
  theme_WAC(1) +
  ylab(bquote('temperature [' * degree * 'C]')) +
  xlab('time after 8:01:00 UTC [s]') + 
  theme(legend.direction = 'vertical',
...
if (generateFigures || !(file.exists('figure/fig8.pdf'))) {
  print(gb + annotate(
    'text',
    x = DVOCALS$Time[getIndex(DVOCALS, 80100)] + 0.6,
    y = 15.705, #17.12,
    label = '(b)',
    size = 6
  ), vp = vp2)
...
(a): An unheated sensor, VOCALS project, flight 3, time after 8:01:00 UTC; (b): A heated sensor, VOCALS project, flight 3. time after 8:01:00 UTC. {\label{fig:ATATF}}}
\end{figure}
...
illustrate that the erroneous fluctuations in the standard measurements
...
significant for the heated sensor. Most of these rapid fluctuations do not arise from real fluctuations in ambient air temperature, but instead from erroneous subtraction of the unfiltered rapid fluctuations in dynamic heating.

# The Flux Density of Sensible Heat\label{sec:flux-density}
...
corrections to the air temperature after compensation for dynamic
...
if corrections for time response are applied instead to the measurement of the recovery
...
DS <-
  getNetCDF(file.path(Directory, 'SOCRATES/SOCRATESrf15h.nc'),
...
DS$TASX <- SmoothInterp(DS$TASX, .Length = 0)
DS$Q <- DS$TASX ^ 2 / 2010
DS$ATX <- DS$ATF1  # NOTE: ATX, originally heated, is overidden here by the
                   # measurement from the unheated sensor.
...
DS$QF <- as.vector(signal::filter(ARG, DS$Q))  # REF4a
DS$QF <- ShiftInTime(DS$QF, .shift = -Lshift * 40, .rate = 25)  # REF4a
DS$AT <- DS$ATX + DS$alphaR * (DS$Q - DS$QF)  # REF4a
DS$AT <- SmoothInterp(DS$AT, .Length = 0)
DS$RTF1 <-
...
aV <- ParamFS1$a  # Adjusted from ParamFS
tau1V <- ParamFS1$tau1
tau2V <- ParamFS1$tau2
DS$DTMDT <-  (c(0, 8 * diff(DS$RTF1, 2), 0) -
                      c(0, 0, diff(DS$RTF1, 4), 0, 0)) * Rate / 12
...
\begin{figure}
```{r SOCp1, include=TRUE, fig.height = 4.1, out.width = "12cm"}

LTphaseC <- function(f, P) {  # LTphase but returns complex transfer function
  ## f=frequency; P=Param
  tau1 <- P$tau1
  tau2 <- P$tau2
  a <- P$a
...
for (.data in list(DS1, DS2, DS3)) {
  ...
  ## supersede with direct use of ATC:
...
tau1V <- ParamFS1$tau1
tau2V <- ParamFS1$tau2
DCX$DTMDT <-  (c(0, 8 * diff(DCX$RTF1, 2), 0) -
                      c(0, 0, diff(DCX$RTF1, 4), 0, 0)) * Rate / 12
fSS <- function(y, i) {  # Eq. Ts3
...
if (generateFigures || !(file.exists('figure/fig9.pdf'))) {
  cairo_pdf('figure/fig9.pdf')
  grid.newpage()
  position <- c(1, 2)
  h <- 0.85 / position[2]
  yp <- 0.05 + (position[1] - 0.5) * h
  ...
  CS.corrected <- plotCS(CS_SOCRATES, Units, spans, fL_SOCRATES, wavelengthLimit,
                    smoothBins = sBins,
                    xlim, ylim, CSprevious = CS.orig_SOCRATES,
                    printTitle = FALSE,
...
1. The cospectra have been smoothed using width-3 moving averages for frequencies above \unit{0.01\,Hz},
...
4. The grey line labelled "exceedance" is
...
grey lines, were calculated using dynamic-heating terms
filtered to match the responses of the sensors, but otherwise were
not corrected. About 33\% of the flux contributed by wavelengths
smaller than \unit{2\,km} would be missed without correction for time 
response as represented
by the transfer function, as shown by the solid grey lines.
The underestimation is particularly
...
sensor underestimated the flux by about 21\%.
The magnitude of the correction here is larger than their estimated
error, but the flight speed of the aircraft was about 30\% greater
than that of the aircraft they used so a larger error would be expected.
...
```{r plotCS2, include=TRUE, echo=FALSE}
...
# source('chunks/plotCS2.R')
source('chunks/plot2CS.R')
...
above \unit{1\,Hz}, where the sensor may respond incompletely to the fluctuations and underestimate the flux. 
...
```
\begin{figure}
```{r simF, include=TRUE, out.height = '5cm', out.width = "8.3cm"}
...
CS.corrected <- plot2CS(CS, Units, spans=121, fL, wavelengthLimit, 
                   smoothBins=sBins, 
                   xlim=xlim, ylim=ylim, 
                   plotFigure = FALSE) 
CS.G <- plot2CS(CS.G, Units, spans=121, fL, wavelengthLimit, 
                   smoothBins=sBins,
                   xlim, ylim, CSprevious = CS.corrected, 
                   plotFigure = FALSE,
                   showNegative = FALSE)
CS.X <- plot2CS(CS.orig, Units, spans=151, fL, wavelengthLimit, 
                   smoothBins=sBins, 
                   xlim=xlim, ylim=ylim, 
                   showNegative = FALSE,
                   plotFigure = FALSE,
...
g <- g + geom_path(data=data.frame(x=rep(fL, 2), y=ylim), aes(x=x, y=y), linetype=2, linewidth = 1.3)
...
if (generateFigures || !(file.exists('figure/fig10.pdf'))) {
  cairo_pdf('figure/fig10.pdf')
...
The result for the cospectrum that would be measured
...
corrected exceedance distribution in Fig.\ \ref{fig:simF} (dash-dot
...
the result should be smoothed, here using a low-pass filter with \unit{2\,Hz} cutoff
frequency. 
...
\begin{figure}
```{r sampleFFT, include=TRUE, out.height = "5cm", out.width = "12cm", messages = FALSE}
...
if (generateFigures || !(file.exists('figure/figA1.pdf'))) {
  cairo_pdf('figure/figA1.pdf')
  print(g1, vp = vp1)
  print(g2, vp = vp2)
  invisible(dev.off())
}
