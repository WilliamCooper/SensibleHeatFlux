...
\Author[][cooperw@ucar.edu]{William A.}{Cooper}
\Author[]{Adriana}{Bailey}
\Author[]{Joshua}{Carnes}


\affil[]{Earth Observing Laboratory, National Center for Atmospheric
Research 80307-3000 Boulder, CO, United States}

\runningtitle{Measuring Sensible-Heat Flux}

\runningauthor{Cooper et al.}


\correspondence{William A.\ Cooper\ (cooperw@ucar.edu)}
...
aircraft, a Gulfstream V (hereafter, GV) and a Hercules C-130. Brief
descriptions of these aircraft are available at these sources:
\citet{GV}, \citet{C130}. The temperature sensors producing the
measurements are in widespread use so these results should have broad
applicability. Some aspects of the uncertainty limits associated with
these measurements of temperature are included in an NCAR Technical Note
\citep{cooper2016ncartn}, where it was estimated that the standard
uncertainty in measurements of temperature from the GV is about
\unit{0.3^{\circ}C}. However, this estimate does not apply when the
temperature changes rapidly. \citet{FrieheKhelif1992} and
\citet{LawsonRodi1992}, among others, provide reviews of the evidence
for delayed response of standard sensors. The unheated sensor referenced
above and cited in these studies has been used widely as a
fast-responding sensor so it will be a focus of this study.
...
then repeated after recalculating \(Q\) using Eq.~\eqref{eq:DHterm} and
that new prediction of \(T_{r}(t)\), and the results were fitted again
by adjusting the fit parameters. Only insignificant changes, much
smaller than the quoted uncertainties, resulted from this iteration.
...
parameter \(a\) would indicate that no heat is transferred from the air
to the sensing wire, but instead all is transferred from the support to
the wire, while the support has a relatively slow characteristic
response.
...
Because the three-parameter fit distorted the measured result, fits in
the logarithm of the frequency were used to provide a better
representation of the measurements, as shown by the dashed green lines
labelled ``lfit''. Those fits are given by these equations and
coefficients, with \(x=\log_{e}(\nu/\nu_{0})\) where \(\nu\) is the
...
The new approach followed here is to filter the dynamic-heating term to
match the response of the sensor, so that only the actual sensor
response to dynamic heating is subtracted from the measurement. This is
made possible by the assumed linearity in response of the sensor. The
proposed filtering removes a significant source of erroneous
fluctuations present in many measurements of temperature made from
research aircraft.

The filtered response to dynamic heating expected for the temperature
sensor has been obtained from the transfer function in three ways: by
integration of the differential equations (Eqs.~\eqref{eq:Ts} and
\eqref{eq:Tm}) applied to \(Q\) instead of \(T_r\), by Fourier
...
sensor. The digital filter has the advantage that it can be applied to
an entire flight and avoids the need to segment and patch results from
the Fourier transform.

A revised estimate of the ambient air temperature was then calculated
using Eq.~\eqref{eq:QprimeCorr} and the filtered dynamic-heating term
...
at high frequency vs.~that using the standard dynamic-heating term. Even
at \unit{1\,Hz}, filtering removes a significant and erroneous
contribution to the variance in the measured ambient temperature.
...
heated sensor. Most of these rapid fluctuations do not arise from real
fluctuations in ambient air temperature, but instead from erroneous
subtraction of the unfiltered rapid fluctuations in dynamic heating.
...
They can be requested from the National Science Foundation (NSF)
...
package named Ranadu \citep{ranadu2.7}, containing auxillary functions,
...
U.S. National Science Foundation.} %% optional section
...
Data provided by NSF NCAR EOL. https://data.eol.ucar.edu/. The data were
collected using NSF's Lower Atmosphere Observing Facilities, which are
managed and operated by NCAR's Earth Observing Laboratory. Measurements
used here (\citet{VOCALS2011}, \citet{SOCRATES2019}, \citet{CSET2017})
were collected in research projects (\citet{wood2011vamos},
\citet{albrecht2019CSET}, \citet{mcfarquhar2021observations}) that used
the NSF/NCAR C130 (\citet{C130}) and GV (\citet{GV}) research aircraft.
Project descriptions and additional information are available at the EOL
data site: \href{https://data.eol.ucar.edu}{https://data.eol.ucar.edu}.
...
with ``RMarkdown'' (\citet{XieEtAl2018RMarkdown,Allaire2026}) to
generate this paper.
