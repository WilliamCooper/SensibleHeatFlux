...
where FTP access can be requested; see also the specific links in
the bibliography of the paper. The details of the processing algorithms
...
The .Rmd file for the paper is basically \LaTeX{} text, generated for simplicity
using \LyX , exported to .Rnw format, converted to .Rmd using the
``mdsr'' package for R (\citet{mdsrBaumer2019}), and then placed
into the Copernicus template provided by Daniel N$\ddot{\mathrm{{u}}}$st
which uses the ``rticles'' package for R (\citet{rticles}). After
some necessary modifications to the output from ``mdsr'', this was
processed to .pdf format using RStudio (\citet{RStudio2021}). RMarkdown
is described by \citet{XieEtAl2018RMarkdown}. Within the .Rmd file
or within the .lyx file there are ``chunks'' of R code (\citet{Rlanguage}),
...
The Ranadu package as used here is preserved at this reference: \citet{ranadu2.7}.
...
\href{https://github.com/WilliamCooper/Ranadu/blob/master/inst/RanaduManual.pdf}{this URL}. 
...
In the code used for this paper, this was replaced by ``plotCS()'',
a function that incorporated much of the same code. This was done
partly to facilitate placing the two panels of ``fig9.pdf'' into
a single plot.
...
Plots of variance spectra, as shown in Figs.~3 and 6, were generated
using the Ranadu function ``VSpec()''. There is a brief discussion
of this function in ``RanaduManual.pdf'' p.~39, available at \href{https://github.com/WilliamCooper/Ranadu/blob/master/inst/RanaduManual.pdf}{this URL}
or in the Zenodo reference. A tutorial Shiny-app is also available
\href{https://ncar-eol.shinyapps.io/VarSpec/}{here} that discusses
plotting spectral variance and describes some of the options available
in ``VSpec()''. R help facilities also provide documentation once
the ``Ranadu'' package is installed.
...
carry forward the variable attributes so this retains them by transfers
...
``Heat flux'' is therefore at best redundant because heat is necessarily
...
to my knowledge that term has never been used or defined. "Temperature flux
density" might be a better term. In the end
I decided to stay with ``flux of sensible heat'' because that seems more
likely to convey the aim of the paper. (I have the same
...
``Eq.~(n)'' as in that paper, while references to equations in
this workflow document use the format ``(n)''.} for a sinusoidal input, as Eqns.~(7)--(12), and determines the
...
the governing differential equations to show that those equations
...
to include in the paper and may be unnecessary here also, but perhaps
it will be of interest to someone wanting to replicate or extend this
work. In particular, it may be useful if additional terms are needed
to describe the time response.

\subsection{Re: Section 2.1}

Here is exhaustive detail on the Laplace-transform solution. I went
into this much detail to be convinced that the proposed solution
was valid. This material is mostly superfluous because the solution
has been verified several ways, so most readers can skip this subsection.
This material will be unnecessary for those already expert in Laplace
transforms because they will be able to see the solution immediately
from the differential equations, as co-author Carnes has demonstrated. 
...
\]
which is also zero. These results then verify that the differential
equation is satisfied for the case where the true signal is a continuous
sine wave.

\paragraph{Comments regarding Fig.~1 in the paper:}
...
of ambient temperature, they enter the estimate of dynamic heating. For that
...
reference to the ambient temperature. This makes possible the iterative process used later in this section.

The formula used to calculate the dynamic-heating variable $Q$ is
as follows:
...
where $V$ is the airspeed, $c_{p}$ the specific heat of air, $M$
the Mach number, $T_{r}$ the (true) absolute recovery temperature,
and $\alpha_{r}$ the recovery factor. The substitution $c_{v}=\frac{5}{2}R$
has been used, where $R$ is the gas constant for air. Initially $T_{m}$
was used in place of $T_{r}$, but once valid transfer-function parameters
were found the measurements were corrected to obtain better values
for $T_{r}$. The change in the results was insignificant.

\subsection{Re Sect.~2.3, Data Sources}

No additional information seems needed here. Someone wanting to reproduce this work can request the data files as described previously. This work is all based on publicly available data sets that are preserved with associated Document Object Identifiers (DOIs). See the paper for citations for the data sets used, some of which are used again in this workflow document.

\subsection{Re Sect. 2.4, Fits to the Measurements}

The initial few paragraphs are justification for using the dynamic-heating term to characterize the response of the temperature sensor. In the course of investigating this, some plots were constructed that were not included in the final paper. An example was the variance spectra for the two terms on the right side of Eq.~(16). That figure is included here as Fig.~\ref{fig:Qprime}. It shows the extent to which the second term dominates over the first for the entire frequency range. That helps justify attributing the variance in dynamic heating primarily to the variance in the Mach number, so that to a first approximation the true variance in the temperature-dependent first term in Eq.~(16) can be neglected. 
...
function and cannot be used to determine it. For this reason, regions and frequency intervals were sought where it appeared that the fluctuations in $T_{m}$ arose primarily from fluctuations in $Q$\@. This should be the case where the intensity of turbulence is high and would produce much larger fluctuations in $T_{m}$ than the existing fluctuations in ambient temperature. 
...
settled upon to avoid significant distortion of the mean when calculated for a region where the variable changes rapidly, as for example approaching 10~Hz in this figure. Too large an interval, with the average defined at the mid-point of the interval, could distort the means toward higher values compared to the true distribution and hence would bias the determination of response variables. These plots have a more convincing appearance when a smaller number of bins is used, and became very
...
in Fig.~2b. (The measurement used
...
The plotted ``best'' theoretical response was generated by ``LTphase()''
...
ratio. Figure~2a shows the amplitude ratio (the square roots of the ratio of variance spectra) or the ``gain''. The plotted frequencies are limited
to those above $10^{-2}\mathrm{Hz}$ because, below that frequency,
bins had too few independent measurements to provide good statistics,
although the available measurements below 0.01~Hz were consistent
with zero phase shift as would be expected from the theoretical response.
The frequency interval was also limited to <12~Hz because the highest-frequency
points in the plot otherwise departed significantly from the trend
by being much higher (about $-43^{\circ}$) and so appearing to be
outliers that should be excluded from the fit. The highest frequency
included in the plot also shows some possible deviation and maybe
should also be excluded.

For the fit, the calculation of the chi-square included the
...
significantly and systematically from the theoretical prediction.\footnote{The reason may be that, for lower frequencies, the fluctuations are
influenced by real fluctuations in temperature not associated with
dynamic heating while for higher frequencies there appears to be spurious
variance in the measurement of temperature. This is discussed in more
detail in a subsequent paragraph.} The R function ``optim()'', part of the standard ``stats'' package,
...
outside the .Rmd code by changing the dynamic-heating term in the
six calls to CohP (lines 871 to 876) to ``DH2'', which was calculated
by using the corrected ``RT'' variable in place of the measured
variable (``TTRR''). The result produced only a small change to
the fit parameters, within estimated uncertainty, even after only
one iteration so the iteration procedure was not included in the processing
code. (The values obtained after iteration were those quoted in the
paper.)

\textbf{\emph{\uline{Investigation of the high-frequency amplitude
ratio in Fig.~2a:}}}

A troubling aspect of Fig.~2a in the paper is the departure from
the fit for frequencies above about 3~Hz. The correction to the recovery
temperature discussed in the previous paragraph was used to show that
this did not correct the problem. The amplitude ratio is the ratio
of the fluctuations in $T_{m}$ (TTRR) or, after correction, $T_{r}$,
to those in $Q$, so the explanation could be either too large fluctuations
in $T_{m}$ (e.g., from noise) or too small fluctuations in $Q$ (e.g.,
from attenuation of the signal). $Q$ depends on $V^{2}$ so the variance
spectrum for $Q$ would be expected to show a -5/3 slope on a logarithmic
plot if that is the case for $V$. It is less clear what slope to
expect for the measured recovery temperature TTRR, although in an
inertial subrange the air temperature might be expected to exhibit
-5/3 slope. The manuscript argues that a filtered version of dynamic
heating should be used, so it is useful to correct the dynamic heating
to see the shape of the resulting air temperature. This is shown as
AT in the following plot (Fig.~4).
...
g <- g + theme_WAC()
g <- g + geom_path(data=data.frame(x=c(1e-2,10), y=c(2.e-3, 2.e-5)), aes(x=x, y=y), colour='darkorange')
...
RTX <- RTX * E$Amp^2
# g <- g + geom_path(data=data.frame(freq=freq, RTX=RTX), aes(x=freq, y=RTX), colour='red', lty=2)
...
<<include=TRUE, fig.cap='Variance spectra for the measured recovery temperature (TTRR) and for dynamic heating (Q) for the six flight legs used for Fig. 2. The variance spectrum for the air temperature after correction for dynamic heating as in the manuscript (AT) is also shown. The orange line is a reference line with the expected -2/3 slope.'>>=

print(g)
@

The variance spectra for $Q$ and TTRR both seem to display high-frequency
problems:
\begin{enumerate}
\item The spectrum for $Q$ appears too small at high frequency. The expectation
is that this should have $-2/3$ slope on this weighted plot, as indicated
by the orange reference line. The spectral density for $Q$ starts
to depart below the expected slope for frequencies above about 5 Hz,
and there may be excess variance around 5 Hz.
\item The deduced response function extrapolated from lower-frequency measurements
predicts that TTRR should be attenuated at high frequencies, but the
measurements do not show attenuation as expected.
\end{enumerate}
...
(not shown). This is not the expected behavior for a sensor having
the time response otherwise found in this study; instead, the expected
shape would be $-2/3$ slope for low frequency and substantial attenuation
of the measurement at high frequency. 

The next plot (Fig.~5) shows, as RM, a calculated prediction of how
the variance spectrum of the recovery temperature would look if the
true air temperature were as shown by the orange line. The result
resembles the shape of the measurement (TTRR) reasonably for frequencies
below about 3 Hz but shows a significant departure above 3~Hz, matching
the decrease in the filtered dynamic-heating term (QP). 
...
g <- g + theme_WAC()
g <- g + geom_path(data=data.frame(x=c(1e-2,10), y=c(1.e-3, 1.e-5)), aes(x=x, y=y), colour='darkorange')
# g <- g + geom_path(data=data.frame(freq=freq, RTX=D$), aes(x=freq, y=RTX), colour='red', lty=2)

@

<<include=TRUE, fig.cap='The predicted measurement (RM) if the air temperature has the spectrum shown as the orange line and the modified dynamic-heating term (QP) is added. The actual measurement is also shown (TTRR).'>>=

print(g)
@

Figure.~2a in the paper is based on the ratio of amplitudes, but
if there is noise in one or both of TTRR and Q then that will make
a false contribution to the ratio because what is needed is how TTRR
responds to Q. Therefore it may be more appropriate to consider the
part of TTRR that is ``caused'' by Q, which will be reflected in
the coherence. This suggests scaling the spectrum of TTRR to represent
only the part of the variance attributable to Q. That is: Find the
variance represented by $P(\nu)\mathrm{Coh}(\nu)$ where $\mathrm{Coh}(\nu)$
is the squared coherence between TTRR and Q. Then use the ratio between
this spectral amplitude and that of Q. This then assumes that the
measurement of Q does not contain noise and is not attenuated.

\begin{figure}
\begin{centering}
\includegraphics[width=0.85\paperwidth]{Tcoh}
\par\end{centering}
\caption{Variance spectra for the measured recovery temperature (TTRR), dynamic
heating (Q), and the measured recovery temperature weighted by the
coherency (TCOH).}

\end{figure}

The resulting figure (Fig.~6) shows that this leads to a reasonable
decrease in the resulting spectrum (plotted here as TCOH) at high
frequency. This doesn't explain why TTRR has excess variance here,
but it does confirm that the excess is not driven by dynamic-heating
fluctuations so it supports excluding that region from the fit.

The source of the excess variance in the measured recovery temperature
has not been identified, but it appears that this is the primary cause
of the high-frequency departure from theory in Fig.~2a of the manuscript.
In this study, it has been assumed that this departure is spurious
and the contribution to the amplitude ratio above 3~Hz should be
ignored when fitting the transfer function to the measurements. For
this reason, the fit used only the amplitude ratios for frequencies
below 3~Hz (although the phase was used up to 12~Hz because it seemed
reasonable in this extended range).

\textbf{\emph{\uline{Extension to GV measurements}}}

An earlier version of this paper included a similar evaluation that
used measurements of an unheated sensor on the GV\@. The .Rmd file
includes code for this, but the results were not included in the paper
because the results were apparently less constrained by the measurements
than were those from the C-130. The relevant code chunk is labeled
``GVcheck''. That code will generate figures similar to Fig.~2
in the paper and will fit to the observations to determine the fit
parameters. The chunk argument ``include = FALSE'' causes the plots
to be omitted.
...
added by activating this branch. The results are not included in the
summary of parameters (Table~3). Here is the text that would have
been included in the paper but was excluded:
...
to any fluctuations above 1~Hz, as shown in Fig.~7. 

\begin{figure}
\begin{centering}
\includegraphics[width=0.9\textwidth]{figure/VphaseW-1}
\par\end{centering}
\caption{Phase lag of recovery temperature vs.~dynamic heating, for the measurements (error bars) from a Rosemount
heated sensor, and the theoretical response for the best-fit parameters representing the measurements with frequency below 1~Hz (green line).}

\end{figure}

The measured phase between the measured recovery temperature and the
calculated dynamic heating is shown in Fig.~8. Above 1~Hz, the lack
...
these values are the basis for the theoretical lines in Figs.~8 and
9.\footnote{The fit was constrained to keep $a\geq0$. Still smaller values of
...
temperature ($T_m(t)$) from the heated Rosemount sensor to that
...
The phase shift leads to these conclusions for the Rosemount heated
sensor:
\begin{enumerate}
...
are involved or that the representation of time response is more complicated. This also indicates that wrong-sign contributions to
...
The transfer function for this sensor is shown in Fig.~10. It does
...
The first try was based on WE-CAN data, but the results did not seem
to give a response dominated by dynamic-heating forcing. Those analyses were superceded by a set of flight
...
paper using the same procedures. The fit provided by Eqs.~(17)--(18)
...
and \citet{khan2005fluid} indicate that the Nusselt
...
were difficult to find, but one candidate was from the ``DC3'' project \citet{DC3},
...
predicts about 0.033. The results aren't very different but they are
...
in $\tau_{2}$; the results appear to favor a smaller value.
Using the expected dependence for the sensor is probably not appropriate
for the support, so no change was suggested for the second time constant. It would be useful to find more information on that time constant; this could be a useful topic for future work.

\subsection{Omitted Sect. 2.5, response to a step change}

This section was omitted from the final paper and was removed from the final SensibleHeatFluxAMT.Rmd file. The VOCALS project
provided a good opportunity to search for discontinuities topping
the marine boundary layer because most flights included numerous climbs
and descents through that structure. A large number of these were
examined in a search for sharp transitions, but almost all showed
...
case, from about 50 examined. Some other projects were also considered,
but good cases where the unheated Rosemount was in use were not found.
It appears that this is a difficult way to test the response characteristics
of the sensor, although other situations (terrestrial boundary layer,
frontal surface, etc.) might provide better opportunities.

An early version of this plot produced good agreement between the
observations and the expected measurement, but that agreement was
based on erroneous parameters for the response. The corrected results
are reproduced here:
...
The section has now been omitted because
it no longer provides useful support for the transfer function as
determined here. Furthermore, the top of the BL is cloudy, so I worry
about the complication that sensor wetting may introduce. 

<<label='tfit', include=TRUE, echo=FALSE, fig.height=6, out.width = "8.3cm", fig.cap='(blue dots): Recovery temperature measured at 25 Hz during descent through an inversion capping the marine boundary layer, from VOCALS flight 3. The dotted black line shows the estimated true recovery temperature, featuring a sharp change at the top of the boundary layer and a dry-adiabatic lapse rate in the marine boundary layer. The prediction using the parameters listed in the text is shown as the magenta line, and the dashed brown line is the calculated temperature of the support that contacts the sensing wire.'>>=

Data <- getNetCDF(file.path(DataDirectory(), 'VOCALS/VOCALSrf03h.nc'),
                  c('PALT', 'GGALT_NTL', 'ATX', 'ATRL', 'ATRR', 'ATWH', 'DPXC', 'PSXC',
                    'QCXC', 'TTRR'),
...
Some prior studies have used sharp temperature changes in the atmosphere,
for example from climbs through the inversion at the top of a boundary
layer, to study the time response of airborne thermometers. A search
of available climbs and descents through inversions capping the marine
boundary layer found that almost all have measurable structure and
are not discrete transitions. The best case found after a search of
candidate passes through inversions in the VOCALS project is shown
in Figure~11 from VOCALS flight~3, where the aircraft descended
through the top of the marine boundary layer at approximately $5\,m\,s^{-1}$.
The assumed measurand history, for an assumed discrete discontinuity
at the inversion and a well-mixed cloudy boundary layer below, is
that shown by the dotted black line.

The predicted time response from Eqns.~(4) and (5) for the time constants
listed in Table 3 for the unheated sensor (adjusted for air density
and flight speed) is shown as the magenta line in Fig.~11. The observations
initially change as fast as the predicted response, but after about
0.2~s the measurements change significantly slower than predicted.
The predictions and measurements only agree if the time constants
are increased by about a factor of 2.5. It appears that, like all
other boundary-layer tops observed in this project, the actual structure
is modified by mixing at the cloud top and is not the discrete discontinuity
assumed in these calculations. Nevertheless, the observations provide
some support for the conclusion that the time response is, within
about a factor of 2.5, about as estimated from the transfer function. 

\rule[0.5ex]{1\columnwidth}{1pt}

(this is the end of the section re the step-change response, excluded
from the paper.)
\newpage
...
excluded from the final version of the paper. 
...
the measured temperature during the speed run. (Measurements from the DEEPWAVE experiment, \citet{DEEPWAVEhr}.)
...
to incorporate this filtering, and it adds a new measurement of ambient
...
Figure 9 was generated using some special code developed for this
purpose, function ``plotCS()'' loaded in the ``initial3'' code
chunk from the ``chunks'' subdirectory. A related plot function
is available from ``Ranadu::flux()'' but in order to combine the
two plots from SOCRATES and CSET into one plot ``plotCS()'' was
used to place the appropriate plots into viewports that were passed
to the function. There is an inactive remnant call to ``flux()''
in the ``SOCp1'' code chunk that was used originally but has been
replaced by calls to plotCS(). The earlier calls to ``Ranadu:CohSpec)''
...
The following points apply to the ``plotCS()'' function used to
calculate the cospectrum of temperature and updraft for the SOCRATES
measurements. The analogous procedure was followed also for CSET.
\begin{enumerate}
\item A data.frame containing the needed 25-Hz measurements from SOCRATES
flight~15, including the updraft and the variable ATF1 representing
the air-temperature measurements from an unheated sensor, was constructed
for the time segment from 5:55:00 to 6:20:00. Because the recovery
temperature (normally RTF1) was not included in the archived data
file, it was reconstructed from ATF1 and the measured dynamic-heating
term. The routine ``SmoothInterp()'' was used to guard against missing
variables by interpolating to fill those missing values. This applied
to about 4\% of the points and resulted from a time-synchronization
error in the original processing.
\item The first argument to ``plotCS()'' is a data.frame that includes
a vector of frequencies and the corresponding values for the measured
cospectrum. The cospectrum is generated from the real part of the
product of the Fourier transform of the measured recovery temperature
and the complex conjugate of the Fourier transform of the measured
updraft. Fourier transforms are obtained using the R routine ``fft()''.
Before transforming, the measured air temperature (AT) was corrected
as described in Sect.~3 of the paper by adding the original dynamic-heating
correction and then subtracting the filtered dynamic-heating correction.
(See the lines labeled REF4a in the /Rmd file.) The Fourier transform
of the temperature is then corrected using the transfer function so
that the cospectrum supplied by the argument CS is corrected for the
time response of the temperature sensor.
\item Another argument, ``CSprevious'', provides the uncorrected cospectrum
and is used to generate the dashed exceedance line representing the
original measurements. 
\item Some other variables in the code, such as ``.B = ATC'', are remnants
from various explorations not in the final paper and can be ignored.
\item The argument returned from ``plotCS()'' is the original cospectrum
with the exceedance function as described in the paper and with added
attributes that specify the cospectrum after smoothing, the net flux,
and the flux contributed by wavelengths smaller than the wavelength
limit provided to the function. These values of the fluxes were used
to determine some of the estimates of flux density quoted in the paper.
\end{enumerate}
...
documented \href{https://drive.google.com/file/d/1BvlvtFkixW9IrIifsXqFKoJ7HaehHYvX/view?usp=sharing}{here}.
...
method used in the document referenced above. However, there appeared
...
Eqns.~(4) and (5) in the paper is straightforward, but the reduction
...
<<Inverarity, include=TRUE, fig.cap='Comparison between the solution developed in the paper (green line, RT) and that proposed by Inverarity (dotted orange line, RTI). The original measurement is shown as TTRR (blue line), and the calculated temperature of the support is shown as Ts (dashed cyan line).'>>=
...
DT %>% select(Time, TTRR, RT, RTI, Ts) %>% 
  ...
  plotWAC(lwd=c(2,3,4,3), 
          col=c('blue', 'forestgreen', 'darkorange', 'cyan'), 
          lty=c(1,1,3,2))

@

Figure~\ref{fig:Inverarity} shows the equivalence between the Inverarity
solution and the one proposed in this paper, for corresponding response
functions with parameters equal to those determined for the unheated
sensor.
...
are removed and then the time series are padded at the start and end
...
\addcontentsline{toc}{section}{\refname}\bibliography{WAC.bib}
