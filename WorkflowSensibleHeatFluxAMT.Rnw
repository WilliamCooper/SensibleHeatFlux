...
of this function in ``RanaduManual.pdf'' p.~39, available at \href{https://github.com/NCAR/Ranadu/blob/master/inst/RanaduManual.pdf}{this URL}
...
into this much detail to be convinced that the proposed solution proposed
...
process used later in this section.
...
ratio. Figure~2b shows the phace. The plotted frequencies are limited
...
Figure~2a was generated in similar fashion, using the ratio of amplitudes
...
six calls to CohP (lines 869 to 874) to ``DH2'', which was calculated
...
This section was omitted from the final paper. Tentative text and
code still exist, commented, in the .Rmd file. The VOCALS project
...
temperature rather than the ambient temperature and to correct an
error in the calculation. The section has now been omitted because
...
Most prior studies have used sharp temperature changes in the atmosphere,
...
from the paper, except for Fig.~11 that follows.)
