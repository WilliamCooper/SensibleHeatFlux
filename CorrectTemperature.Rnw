...
\bigskip
The processing script described herein (``CorrectTemperature.R'') adjusts the measured recovery temperature to compensate for the known response of the sensor, before correction for dynamic heating. In addition, the script produces a new variable with the original (uncorrected) measurement of recovery temperature but with the dynamic-heating term revised to compensate for the response time of the temperature sensors. If the temperature sensor responds slower than the measurement of dynamic heating, the dynamic-heating term should be filtered to compensate for this difference in response times before it is subtracted from the measured recovery temperature. Otherwise a false signal may be introduced into the measured ambient temperature. This is described in \citet{CooperEtAl2022}.
...
how the recommendations from that paper can be implemented in a second-pass processing script. It
is hoped that this note will be useful to anyone needing to use and
maintain the processing script.
...
structure $T_{s}(t)$. As given in \citet{CooperEtAl2022}, they are:
...
transfer function as developed in \citet{CooperEtAl2022}, $H(\omega)$,
...
differential equations as might be needed if the air density and/or
Mach number varies over the block of measurements.
...
noise, the result apparently needs smoothing, here via a low-pass
filter with 2~Hz cutoff frequency. This is acceptable because the
heated sensor has only insignificant response at such frequencies.

\subsection{The Treatment of Dynamic Heating}

As described by \citet{CooperEtAl2022}, the standard dynamic-heating
...
dynamic heating; i.e., the \emph{true} recovery temperature. Then subtract the \emph{full} unfiltered dynamic-heating term as measured.
In the frequency-space representation, because $H(\omega)\hat{Q}(\omega)/H(\omega)=\hat{Q}(\omega)$
...
method \#2. However, the limited examples in \citet{CooperEtAl2022}
...
preserved in many production files, the script reconstructs the recovery
...
The processing code for this script is inserted below as a code chunk
named ``processor''. A processing example will be run when
this file is exported to pdf via pdflatex. The script is available
separately as ``CorrectTemperature.R,'' and that is included in
the CorrectTemperature.zip file that is the project archive for this
processor. That archive is preserved at this URL:\\  
\href{https://github.com/WilliamCooper/SensibleHeatFlux}{https://github.com/WilliamCooper/SensibleHeatFlux}
.
...
probes are not included in the corrected 1-Hz files. A run-time argument
can change that default if it is desired to produce corrected variables
for all the sensors.

The results for 1-Hz files are better, however, if the 25-Hz file
is processed first and the corrected variables are averaged to 1-Hz.
The script has provision to use these values if they are available.
For a 1-Hz file, the script checks if a previous run at 25-Hz produced
a 1-Hz data.frame containing the averaged values, and if so those
values are used instead for the corrected temperature. For that reason,
it is better to process the high-rate file first. An example for
running via Rscript is the following (or CorrectTemperature.R can be
run by selecting "source" from the RStudio display):
...
dynamic-heating term. This removes the excess noise arising from the
...
\subsection{Special Considerations for 1-Hz Files}

The processing script will work for 1-Hz files, but better results
can be obtained by using the corresponding 25-Hz file. For this reason,
when a 25-Hz file is processed, a special data.frame is saved with
the name ProjectFlightLRT.Rdata (for example, VOCALSrf05hLRT.Rdata).
If such a file is available, the script will instead use the values
of variables in that file when processing a 1-Hz file. 

The preferable approach is to process all 25-Hz files first and then
process the 1-Hz files. The new entries in the 1-Hz file will then
be 1-s-average values of the corresponding variables in the 25-Hz
file. See the ``Examples'' section for an illustration of the benefit
...
file and were saved in ``AR.Rdata'', which is loaded at the start
of this CorrectTemperature.R script. Details regarding how this filter
was generated are availabie in the \href{https://github.com/WilliamCooper/SensibleHeatFlux/blob/master/WorkflowSensibleHeatFluxAMT.pdf}{Workflow document}
that accompanies \citet{CooperEtAl2022}. The function ``CorrectTemperature.R''
retrieves the recovery factor from the attribute saved with the variable,
as needed if the recovery temperature is absent and should be recalculated.
It then uses that recovery factor to calculate the dynamic-heating
term. Application of the signal::filter() function will fail is there
are missing values in the time series, so Ranadu::SmoothInterp() is
used to replace missing values by linear interpolation before the
filter is applied. Then the appropriate filter from the reference
paper is applied to the time series via ``signal::filter(AF, Q)''
where ``AF'' is the appropriate filter (dependent on the sensor
and the data rate) and ``Q'' is the dynamic-heating term, including
the recovery factor. The file ``AR.Rdata'' also specifies the appropriate
time shift used for each filter. The function Ranadu::ShiftInTime()''
is used after filtering to restore proper timing to the time series
after filtering. The value returned from this function is the air
temperature corrected by the term ($Q-QF$) where $Q$ is the usual
dynamic-heating term $\alpha_{r}V^{2}/(2C_{p})$ and $QF$ is the
result of applying the filter developed in the reference paper.
...
Examples were shown in \citet{CooperEtAl2022}, Fig.8. It is expected
...
<<vnameF1, include = TRUE, fig.height = 4.5, fig.cap='The measurements from an unheated sensor (ATF1) and the same measurements with the dynamic-heating correction filtered to match the response of the sensor (ATF1F). The lines are mostly indistinguishable in this plot. Data at 1 Hz from SOCRATES flight 15.'>>=
...
Dc %>% selectTime(50500, 50800) %>% select(Time, ATF1, ATF1F) %>% 
      plotWAC(lwd=c(3,1), col=c('red', 'blue'), legend.position = 'topright')
@
...
time response of the sensor and for dynamic heating. The preceding section only filtered the
...
unfiltered full dynamic-heating correction. This is appropriate because
the transfer function restores a best estimate of the true recovery
temperature (the measurand), which includes the effect of dynamic heating without
filtering. These variables then are measurements of the air
temperature, with appropriate corrections for dynamic heating and for
time response of the sensor. 

Figure~\ref{fig:vnameC} shows the original measurement as acchived and the
...
The corresponding results for a 1-Hz file, also for the unheated sensor,
are shown in Figs.~\ref{fig:vnameC1} and \ref{fig:vnameCVS1}. The
unheated sensor responds faster than 1 Hz, so filtering the dynamic-heating
term and correcting for time response both have negligible effect. 
Variables like ATF1C are omitted from the output by default for 1-Hz files
unless they are requested specifically by the runtime argument ``UH1''.

\clearpage{}

For 25-Hz measurements and for the heated sensor, the effect of the
...
where the corrected result from the unheated sensor ATF1C, after
adjustment of the measurement
by -0.1$^{\circ}\mathrm{C}$ and labelled ATF1CM (for
ATF1C-modified), is shown as the thin green line. The time shift was introduced
to facilitate comparison to the corrected
measurement from the heated sensor. The time-response correction
is quite important for this sensor and leads to a measurement almost
...
Da %>% Rmutate(ATF1CM = ATF1C - 0.1) %>%
      ...
      plotWAC(lwd=c(2,3,1), col=c('red', 'blue', 'forestgreen'), lty=c(1,1,1))
@
...
to be as significant as in the case of 25-Hz measurements. The variance spectra
show that the correction removes a significant source of noise, with
frequencies even below 0.1 Hz. For the heated
sensors, the corrected measurements like ATH1C are included in the output files. 
...
A still better way of applying this correction is to resample 25-Hz
measurements at 1 Hz, because the 25-Hz correction appears to be significantly
better than this one. 

<<compare1Hz, include = TRUE, fig.height=4, fig.cap = 'Corrected measurements for an unheated sensor (ATH1) from a 1-Hz data file from SOCRATES flight 15. The original measurement with filtered dynamic heating, ATH1F, is shown as the black line. It is evidently delayed and smoothed relative to the corrected measurement obtained from the unheated sensor, the best estimate of the true ambient temperature which is shown as the red line (with adjustment by $-0.1^\\circ$C to aid comparison to the heated sensor and labelled ATF1M). The green thick line labeled "ATH1C1" is the corrected measurement obtained from the heated sensor using processing applied to the 1-Hz file, while the blue line labeled "ATH1C" is the result of averaging the 25-Hz measurements from the ATH1 sensor to 1 Hz. Both move closer to the reference "ATF1M" line, but "ATH1C" appears to provide a superior correction.'>>=

# DO <- getNetCDF(
#   sprintf('%sSOCRATES/SOCRATESrf15TOLD.nc',DataDirectory()),
#                 standardVariables(c('ATH1', 'ATH1C', 'ATH1F', 'ATF1')))
DO <- getNetCDF(
    sprintf('%sSOCRATES/SOCRATESrf151HzRef.nc',DataDirectory()),
                standardVariables(c('ATH1', 'ATH1C', 'ATH1F', 'ATF1')))
DN <- getNetCDF(                 ## new run using the .Rdata file
  sprintf('%sSOCRATES/SOCRATESrf15T.nc',DataDirectory()),
                standardVariables(c('ATH1', 'ATH1C', 'ATH1F', 'ATF1')))
DN$ATH1C1 <- DO$ATH1C
DN$ATF1M <- DN$ATF1-0.1

# # Average ATH1C to 1 Hz from 25-Hz file:
# ATH1CX <- SmoothInterp(DO$ATH1C, .Length=25)
# for (i in nrow(DN)) {
#   DN$ATH1C[i] <- ATH1CX[which(DO$Time == DN$Time[i])]
# }
DN %>% selectTime(50100,50200) %>% 
       select(Time, ATH1C, ATH1C1, ATF1M, ATH1F) %>%
      plotWAC(legend.position='topright', 
            col = c('blue', 'forestgreen', 'red', 'black'), 
            lty=c(1,1,1,1), lwd=c(3,2,2,2))
@
...
\textsf{\textsc{\textcolor{blue}{Program:}}} & \Sexpr{thisFileName}.Rnw containing the text and code\tabularnewline
