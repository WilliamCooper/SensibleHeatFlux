...
The basis for this script is documented by \citet{CooperEtAl2022}.
...
named ``processor''. chunk. A processing example will be run when
...
processor. That archive is preserved at this URL: \href{https://github.com/WilliamCooper/SensibleHeatFlux}{https://github.com/WilliamCooper/SensibleHeatFlux}
.
...
where the corrected result from the unheated sensor ATF1C,
adjusted by -0.1$^{\circ}\mathrm{C}$ and labelled ATF1CM
to facilitate comparison to the corrected
measurement from the heated sensor, is shown
as the thin green line. The time-response correction
...
\textsf{\textsc{\textcolor{blue}{Program:}}} & \Sexpr{thisFileName}.Rnw produced from the .lyx file\tabularnewline
